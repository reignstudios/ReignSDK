...
namespace BEPUphysics.UpdateableSystems
{
    ...
    public class FluidVolume : Updateable, IDuringForcesUpdateable
    {
        ...
        public void RecalculateBoundingBox()
        {
            ...
            boundingBox = BoundingBox3.FromPoints(points);
            surfacePlaneHeight = points[0].Dot(upVector);
        ...
        void AnalyzeCollisionEntry(int i)
        {
            ...
            if (entityEntry != null && entityEntry.IsActive && entityEntry.entity.isDynamic)
            {
                ...
                {
                    Vector3 force;
                    Vector3.Mul(ref upVector, -gravity * Density * dt * submergedVolume, out force);
                    ...
                    if (FlowForce != 0)
                    {
                        float dot = Math.Max(entityEntry.entity.linearVelocity.Dot(flowDirection), 0);
                        if (dot < MaxFlowSpeed)
                        {
        ...
        {
            BoundingBox3 entityBoundingBox;
            //TODO: Figure out how to best reenable this.
            entityBoundingBox = info.boundingBox;// new BoundingBox3();
            ...
            for (int i = 0; i < samplePointsPerDimension; i++)
            {
                for (int j = 0; j < samplePointsPerDimension; j++)
                {
                    ...
                    if ((submergedHeight = GetSubmergedHeight(info, maxLength, boundingBoxHeight, ref origin, ref xSpacing, ref zSpacing, i, j, out columnVolumeCenter)) > 0)
                    {
                        ...
                        Vector3.Mul(ref columnVolumeCenter, columnVolume, out columnVolumeCenter);
                        Vector3.Add(ref columnVolumeCenter, ref submergedCenter, out submergedCenter);
                        submergedVolume += columnVolume;
                    }
                }
            }
            Vector3.Div(ref submergedCenter, submergedVolume, out submergedCenter);

        }

        void GetSamplingOrigin(ref BoundingBox3 entityBoundingBox, out Vector3 xSpacing, out Vector3 zSpacing, out float perColumnArea, out Vector3 origin)
        {
            ...
            Vector3 right = Toolbox.RightVector;// new Vector3(surfaceOrientationTranspose.X.X, surfaceOrientationTranspose.Y.X, surfaceOrientationTranspose.Z.X);
            Vector3.Mul(ref right, widthIncrement, out xSpacing);
            Vector3 backward = Toolbox.BackVector;// new Vector3(surfaceOrientationTranspose.X.Z, surfaceOrientationTranspose.Y.Z, surfaceOrientationTranspose.Z.Z);
            Vector3.Mul(ref backward, lengthIncrement, out zSpacing);
            perColumnArea = widthIncrement * lengthIncrement;


            //Compute the origin.
            Vector3 minimum = entityBoundingBox.Min;
            //Vector3.TransformTranspose(ref entityBoundingBox.Min, ref surfaceOrientationTranspose, out minimum);
            Vector3 offset;
            Vector3.Mul(ref xSpacing, .5f, out offset);
            Vector3.Add(ref minimum, ref offset, out origin);
            Vector3.Mul(ref zSpacing, .5f, out offset);
            ...
            //{
            //    Vector3 dir;
            //    Vector3.Neg(ref upVector, out dir);
            ...
            //    Vector3.Sub(ref extremePoint, ref origin, out offset);
            ...
            //    {
            //        Vector3.Mul(ref right, 1 - offsetX, out offset);
            //    }
            //    else
            //    {
            //        Vector3.Mul(ref right, -offsetX, out offset);
            //    }

            //    if (offsetZ > .5f * lengthIncrement)
            //    {
            //        Vector3 temp;
            //        Vector3.Mul(ref right, 1 - offsetZ, out temp);
            //        Vector3.Add(ref temp, ref offset, out offset);
            //    }
            //    else
            //    {
            //        Vector3 temp;
            //        Vector3.Mul(ref right, -offsetZ, out temp);
            //        Vector3.Add(ref temp, ref offset, out offset);
            //    }
        ...
        {
            Ray3 ray;
            Vector3.Mul(ref xSpacing, i, out ray.Position);
            Vector3.Mul(ref zSpacing, j, out ray.Direction);
            ...
            if (info.RayCast(ray, maxLength, out rayHit))
            {
                ...
                Vector3.Mul(ref ray.Direction, boundingBoxHeight, out ray.Direction);
                Vector3.Add(ref ray.Position, ref ray.Direction, out ray.Position);
                Vector3.Neg(ref upVector, out ray.Direction);
                ...
                if (info.RayCast(ray, boundingBoxHeight - rayHit.T, out rayHit))
                {
                    ...
                    Vector3.Mul(ref volumeCenter, .5f, out volumeCenter);
