...
namespace BEPUphysics.DataStructures
{
    ...
    public class BoundingBoxTree<T> where T : IBoundingBoxOwner
    {
        ...
        /// </summary>
        public BoundingBox3 BoundingBox3
        {
            get
            {
                if (root != null)
        ...
        public void Add(T element)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    BoundingBox3.Merge(ref node.BoundingBox, ref root.BoundingBox, out root.BoundingBox);
                    Node treeNode = root;
        ...
        public bool GetOverlaps(BoundingBox3 boundingBox, IList<T> outputOverlappedElements)
        {
            if (root != null)
            {
                bool intersects;
                BoundingBox3.Intersects(ref root.BoundingBox, ref boundingBox, out intersects);
                if (intersects)
        ...
        /// <param name="outputOverlappedElements">Indices of triangles in the index buffer with bounding boxes which are overlapped by the query.</param>
        /// <returns>Whether or not any elements were overlapped.</returns>
        public bool GetOverlaps(BoundingSphere3 boundingSphere, IList<T> outputOverlappedElements)
        {
            if (root != null)
            {
                bool intersects;
                BoundingBox3.Intersects(ref root.BoundingBox, ref boundingSphere, out intersects);
                if (intersects)
        ...
        /// <param name="outputOverlappedElements">Indices of triangles in the index buffer with bounding boxes which are overlapped by the query.</param>
        /// <returns>Whether or not any elements were overlapped.</returns>
        public bool GetOverlaps(BoundingFrustum3 boundingFrustum, IList<T> outputOverlappedElements)
        {
            if (root != null)
            {
                bool intersects;
                BoundingFrustum3.Intersects(ref boundingFrustum, ref root.BoundingBox, out intersects);
                if (intersects)
        ...
        /// <param name="outputOverlappedElements">Indices of triangles in the index buffer with bounding boxes which are overlapped by the query.</param>
        /// <returns>Whether or not any elements were overlapped.</returns>
        public bool GetOverlaps(Ray3 ray, IList<T> outputOverlappedElements)
        {
            if (root != null)
            {
                float? result;
                Ray3.Intersects(ref ray, ref root.BoundingBox, out result);
                if (result != null)
        ...
        /// <param name="outputOverlappedElements">Indices of triangles in the index buffer with bounding boxes which are overlapped by the query.</param>
        /// <returns>Whether or not any elements were overlapped.</returns>
        public bool GetOverlaps(Ray3 ray, float maximumLength, IList<T> outputOverlappedElements)
        {
            if (root != null)
            {
                float? result;
        ...
        {
            bool intersects;
            BoundingBox3.Intersects(ref root.BoundingBox, ref tree.root.BoundingBox, out intersects);
            if (intersects)
            {
        ...
        {
            internal BoundingBox3 BoundingBox;
            internal abstract void GetOverlaps(ref BoundingBox3 boundingBox, IList<T> outputOverlappedElements);
            internal abstract void GetOverlaps(ref BoundingSphere3 boundingSphere, IList<T> outputOverlappedElements);
            internal abstract void GetOverlaps(ref BoundingFrustum3 boundingFrustum, IList<T> outputOverlappedElements);
            internal abstract void GetOverlaps(ref Ray3 ray, float maximumLength, IList<T> outputOverlappedElements);
        ...
        internal sealed class InternalNode : Node
        {
            ...
            internal override void GetOverlaps(ref BoundingBox3 boundingBox, IList<T> outputOverlappedElements)
            {
                ...
                bool intersects;
                BoundingBox3.Intersects(ref childA.BoundingBox, ref boundingBox, out intersects);
                if (intersects)
                    childA.GetOverlaps(ref boundingBox, outputOverlappedElements);
                BoundingBox3.Intersects(ref childB.BoundingBox, ref boundingBox, out intersects);
                if (intersects)
                    childB.GetOverlaps(ref boundingBox, outputOverlappedElements);
            }

            internal override void GetOverlaps(ref BoundingSphere3 boundingSphere, IList<T> outputOverlappedElements)
            {
                bool intersects;
                BoundingBox3.Intersects(ref childA.BoundingBox, ref boundingSphere, out intersects);
                if (intersects)
                    childA.GetOverlaps(ref boundingSphere, outputOverlappedElements);
                BoundingBox3.Intersects(ref childB.BoundingBox, ref boundingSphere, out intersects);
                if (intersects)
                    childB.GetOverlaps(ref boundingSphere, outputOverlappedElements);
            }

            internal override void GetOverlaps(ref BoundingFrustum3 boundingFrustum, IList<T> outputOverlappedElements)
            {
                bool intersects;
                BoundingFrustum3.Intersects(ref boundingFrustum, ref childA.BoundingBox, out intersects);
                if (intersects)
                    childA.GetOverlaps(ref boundingFrustum, outputOverlappedElements);
                BoundingFrustum3.Intersects(ref boundingFrustum, ref childB.BoundingBox, out intersects);
                if (intersects)
                    childB.GetOverlaps(ref boundingFrustum, outputOverlappedElements);
            }

            internal override void GetOverlaps(ref Ray3 ray, float maximumLength, IList<T> outputOverlappedElements)
            {
                float? result;
                Ray3.Intersects(ref ray, ref childA.BoundingBox, out result);
                if (result != null && result < maximumLength)
                    childA.GetOverlaps(ref ray, maximumLength, outputOverlappedElements);
                Ray3.Intersects(ref ray, ref childB.BoundingBox, out result);
                if (result != null && result < maximumLength)
            ...
            internal override void GetOverlaps<TElement>(BoundingBoxTree<TElement>.Node opposingNode, IList<TreeOverlapPair<T, TElement>> outputOverlappedElements)
            {
                ...
                if (opposingNode.IsLeaf)
                {
                    ...
                    BoundingBox3.Intersects(ref childA.BoundingBox, ref opposingNode.BoundingBox, out intersects);
                    if (intersects)
                        childA.GetOverlaps<TElement>(opposingNode, outputOverlappedElements);
                    BoundingBox3.Intersects(ref childB.BoundingBox, ref opposingNode.BoundingBox, out intersects);
                    if (intersects)
                ...
                else
                {
                    ...
                    BoundingBox3.Intersects(ref childA.BoundingBox, ref opposingChildA.BoundingBox, out intersects);
                    if (intersects)
                        childA.GetOverlaps<TElement>(opposingChildA, outputOverlappedElements);
                    BoundingBox3.Intersects(ref childA.BoundingBox, ref opposingChildB.BoundingBox, out intersects);
                    if (intersects)
                        childA.GetOverlaps<TElement>(opposingChildB, outputOverlappedElements);
                    BoundingBox3.Intersects(ref childB.BoundingBox, ref opposingChildA.BoundingBox, out intersects);
                    if (intersects)
                        childB.GetOverlaps<TElement>(opposingChildA, outputOverlappedElements);
                    BoundingBox3.Intersects(ref childB.BoundingBox, ref opposingChildB.BoundingBox, out intersects);
                    if (intersects)
            ...
            internal override bool TryToInsert(LeafNode node, out Node treeNode)
            {
                ...
                //    BoundingBox3 merged;
                //    BoundingBox3.Merge(ref childA.BoundingBox3, ref node.BoundingBox3, out merged);
                //    childA = new InternalNode() { BoundingBox3 = merged, childA = this.childA, childB = node };
                //    treeNode = null;
                //    return true;
                //}
                ...
                //    //Just put us with the leaf.  Keeps the tree shallower.
                //    BoundingBox3 merged;
                //    BoundingBox3.Merge(ref childB.BoundingBox3, ref node.BoundingBox3, out merged);
                //    childB = new InternalNode() { BoundingBox3 = merged, childA = node, childB = this.childB };
                //    treeNode = null;
                //    return true;
                //}
                ...
                BoundingBox3 mergedA, mergedB;
                BoundingBox3.Merge(ref childA.BoundingBox, ref node.BoundingBox, out mergedA);
                BoundingBox3.Merge(ref childB.BoundingBox, ref node.BoundingBox, out mergedB);

                Vector3 offset;
                float originalAVolume, originalBVolume;
                Vector3.Sub(ref childA.BoundingBox.Max, ref childA.BoundingBox.Min, out offset);
                originalAVolume = offset.X * offset.Y * offset.Z;
                Vector3.Sub(ref childB.BoundingBox.Max, ref childB.BoundingBox.Min, out offset);
                originalBVolume = offset.X * offset.Y * offset.Z;

                float mergedAVolume, mergedBVolume;
                Vector3.Sub(ref mergedA.Max, ref mergedA.Min, out offset);
                mergedAVolume = offset.X * offset.Y * offset.Z;
                Vector3.Sub(ref mergedB.Max, ref mergedB.Min, out offset);
            ...
            internal override void Refit()
            {
                childA.Refit();
                childB.Refit();
                BoundingBox3.Merge(ref childA.BoundingBox, ref childB.BoundingBox, out BoundingBox);
            }
        }
        ...
        internal sealed class LeafNode : Node
        {
            ...
            internal override void GetOverlaps<TElement>(BoundingBoxTree<TElement>.Node opposingNode, IList<TreeOverlapPair<T, TElement>> outputOverlappedElements)
            {
                ...
                else
                {
                    ...
                    BoundingBox3.Intersects(ref BoundingBox, ref opposingChildA.BoundingBox, out intersects);
                    if (intersects)
                        GetOverlaps<TElement>(opposingChildA, outputOverlappedElements);
                    BoundingBox3.Intersects(ref BoundingBox, ref opposingChildB.BoundingBox, out intersects);
                    if (intersects)
            ...
            internal override bool TryToInsert(LeafNode node, out Node treeNode)
            {
                ...
                BoundingBox3.Merge(ref BoundingBox, ref node.BoundingBox, out newTreeNode.BoundingBox);
