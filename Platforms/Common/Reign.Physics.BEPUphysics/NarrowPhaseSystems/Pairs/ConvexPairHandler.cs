        ...
        public override void UpdateTimeOfImpact(Collidable requester, float dt)
        {
                ...
                )
            {
                ...
                else if (modeB == PositionUpdateMode.Discrete)
                {
                    ...
                    Vector3.Neg(ref collidableA.entity.linearVelocity, out velocity);
                }
                else
                {
                    //Both objects are moving.
                    Vector3.Sub(ref collidableB.entity.linearVelocity, ref collidableA.entity.linearVelocity, out velocity);
                }
                ...
                if (minimumRadiusA * minimumRadiusA < velocitySquared)
                {
                    ...
                    RayHit rayHit;
                    if (GJKToolbox.CCDSphereCast(new Ray3(collidableA.worldTransform.Position, -velocity), minimumRadiusA, collidableB.Shape, ref collidableB.worldTransform, timeOfImpact, out rayHit))
                ...
                if (minimumRadiusB * minimumRadiusB < velocitySquared)
                {
                    ...
                    RayHit rayHit;
                    if (GJKToolbox.CCDSphereCast(new Ray3(collidableB.worldTransform.Position, velocity), minimumRadiusB, collidableA.Shape, ref collidableA.worldTransform, timeOfImpact, out rayHit))
                        timeOfImpact = rayHit.T;
                }
