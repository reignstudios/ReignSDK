...
namespace BEPUphysics
{
    ...
    public static class Toolbox
    {
        ...
        /// <summary>
        /// Matrix4 containing zeroes for every element.
        /// </summary>
        public static Matrix4 ZeroMatrix = new Matrix4();
        ...
        /// </summary>
        public static RigidTransform3 RigidIdentity = RigidTransform3.Identity;

        #region Segment/Ray3-Triangle Tests
        ...
        public static bool FindRayTriangleIntersection(ref Ray3 ray, float maximumLength, ref Vector3 a, ref Vector3 b, ref Vector3 c, out bool hitClockwise, out RayHit hit)
        {
            hitClockwise = false;
            hit = new RayHit();
            Vector3 ab, ac;
            Vector3.Sub(ref b, ref a, out ab);
            Vector3.Sub(ref c, ref a, out ac);
            ...
            Vector3 ap;
            Vector3.Sub(ref ray.Position, ref a, out ap);
            ...
            Vector3.Mul(ref ray.Direction, hit.T, out hit.Location);
            Vector3.Add(ref ray.Position, ref hit.Location, out hit.Location);

            // Compute barycentric coordinates
            Vector3.Sub(ref hit.Location, ref a, out ap);
        ...
        /// <param name="hit">Hit data of the ray, if any</param>
        /// <returns>Whether or not the ray and triangle intersect.</returns>
        public static bool FindRayTriangleIntersection(ref Ray3 ray, float maximumLength, TriangleSidedness sidedness, ref Vector3 a, ref Vector3 b, ref Vector3 c, out RayHit hit)
        {
            hit = new RayHit();
            Vector3 ab, ac;
            Vector3.Sub(ref b, ref a, out ab);
            ...
            switch (sidedness)
            {
                case TriangleSidedness.DoubleSided:
                    ...
                    {
                        Vector3.Neg(ref hit.Normal, out hit.Normal);
                        d = -d;
                    }
                    break;
        ...
        {
            Plane3 p;
            p.Normal = (e - d).Cross(f - d);
            p.Distance = p.Normal.Dot(d);
            float t;
        ...
        /// <param name="p">Plane3 for comparison.</param>
        /// <param name="q">Intersection point.</param>
        /// <returns>Whether or not the segment intersects the plane.</returns>
        public static bool GetSegmentPlaneIntersection(Vector3 a, Vector3 b, Plane3 p, out Vector3 q)
        {
            float t;
        ...
        /// <param name="q">Intersection point.</param>
        /// <returns>Whether or not the segment intersects the plane.</returns>
        public static bool GetSegmentPlaneIntersection(Vector3 a, Vector3 b, Plane3 p, out float t, out Vector3 q)
        {
            return GetLinePlaneIntersection(ref a, ref b, ref p, out t, out q) && t >= 0 && t <= 1;
        }
        ...
        public static bool GetLinePlaneIntersection(ref Vector3 a, ref Vector3 b, ref Plane3 p, out float t, out Vector3 q)
        {
            Vector3 ab;
            Vector3.Sub(ref b, ref a, out ab);
            float denominator;
            ...
            t = (p.Distance - numerator) / denominator;
            //Compute the intersection position.
            Vector3.Mul(ref ab, t, out q);
        ...
        /// </summary>
        /// <param name="ray">Ray3 to test against the plane.</param>
        /// <param name="p">Plane3 for comparison.</param>
        /// <param name="t">Interval along line to intersection (A + t * AB).</param>
        /// <param name="q">Intersection point.</param>
        /// <returns>Whether or not the line intersects the plane.  If false, the line is parallel to the plane's surface.</returns>
        public static bool GetRayPlaneIntersection(ref Ray3 ray, ref Plane3 p, out float t, out Vector3 q)
        {
            float denominator;
            ...
            t = (p.Distance - numerator) / denominator;
            //Compute the intersection position.
            Vector3.Mul(ref ray.Direction, t, out q);
        ...
        public static VoronoiRegion GetClosestPointOnTriangleToPoint(ref Vector3 a, ref Vector3 b, ref Vector3 c, ref Vector3 p, out Vector3 closestPoint)
        {
            ...
            //Vertex region A?
            Vector3 ap;
            Vector3.Sub(ref p, ref a, out ap);
            float d1;
            ...
            //Vertex region B?
            Vector3 bp;
            Vector3.Sub(ref p, ref b, out bp);
            float d3;
            ...
            {
                w = d2 / (d2 - d6);
                Vector3.Mul(ref ac, w, out closestPoint);
                Vector3.Add(ref closestPoint, ref a, out closestPoint);
            ...
            {
                w = (d4 - d3) / ((d4 - d3) + (d5 - d6));
                Vector3.Sub(ref c, ref b, out closestPoint);
                Vector3.Mul(ref closestPoint, w, out closestPoint);
            ...
            w = vc * denom;
            Vector3 abv;
            Vector3.Mul(ref ab, v, out abv);
            Vector3 acw;
            Vector3.Mul(ref ac, w, out acw);
                                                           ...
                                                           out float s, out float t, out Vector3 c1, out Vector3 c2)
        {
            ...
            if (a <= Epsilon)
            {
                ...
                s = 0.0f;
                t = MathUtilities.Clamp(f / e, 0.0f, 1.0f);
            }
            else
            {
                float c = d1.Dot(r);
                ...
                else
                {
                    ...
                    if (denom != 0.0f)
                        s = MathUtilities.Clamp((b * f - c * e) / denom, 0.0f, 1.0f);
                    ...
                    if (t < 0)
                    {
                        ...
                        t = 0;
                        s = MathUtilities.Clamp(-c / a, 0, 1);
                    }
                    else if (t > 1)
                    {
                        //Closest point is after the segment.
                        t = 1;
                        s = MathUtilities.Clamp((b - c) / a, 0, 1);
                    }
                }
            }
        ...
        public static bool ArePointsOnOppositeSidesOfPlane(ref Vector3 o, ref Vector3 p, ref Vector3 a, ref Vector3 b, ref Vector3 c)
        {
            ...
            Vector3.Sub(ref c, ref a, out ac);
            Vector3.Sub(ref p, ref a, out ap);
            Vector3.Sub(ref o, ref a, out ao);
            Vector3 q;
        ...
        {
            Vector3 offset;
            Vector3.Sub(ref point, ref pointOnPlane, out offset);
            float dot;
        ...
        public static void GetPointProjectedOnPlane(ref Vector3 point, ref Vector3 normal, ref Vector3 pointOnPlane, out Vector3 projectedPoint)
        {
            ...
            Vector3 multiply;
            Vector3.Mul(ref normal, t, out multiply);
            Vector3.Sub(ref point, ref multiply, out projectedPoint);
        }
        ...
        public static bool IsPointWithinFaceExtrusion(Vector3 point, List<Plane3> planes, Vector3 centroid)
        {
            foreach (Plane3 plane in planes)
            {
                float centroidPlaneDot;
				var planeRef = plane;
                Plane3.DotCoordinate(ref planeRef, ref centroid, out centroidPlaneDot);
                float pointPlaneDot;
                Plane3.DotCoordinate(ref planeRef, ref point, out pointPlaneDot);
        ...
        public static void GetClosestPointOnTetrahedronToPoint(ref Vector3 a, ref Vector3 b, ref Vector3 c, ref Vector3 d, ref Vector3 p, out Vector3 closestPoint)
        {
            ...
            if (ArePointsOnOppositeSidesOfPlane(ref p, ref d, ref a, ref b, ref c))
            {
                ...
                Vector3.Sub(ref q, ref p, out pq);
        ...
        public static void GetClosestPointOnTetrahedronToPoint(RawList<Vector3> tetrahedron, ref Vector3 p, RawList<int> subsimplex, RawList<float> baryCoords, out Vector3 closestPoint)
        {
            ...
            if (!baryCoordsFound)
            {
                ...
                float abcd = new Matrix4
				(
					new Vector4(tetrahedron[0].X, tetrahedron[0].Y, tetrahedron[0].Z, 1),
                    new Vector4(tetrahedron[1].X, tetrahedron[1].Y, tetrahedron[1].Z, 1),
                    new Vector4(tetrahedron[2].X, tetrahedron[2].Y, tetrahedron[2].Z, 1),
                    new Vector4(tetrahedron[3].X, tetrahedron[3].Y, tetrahedron[3].Z, 1)
				).Determinant();

                float pbcd = new Matrix4
				(
        ...
        public static bool RayCastSphere(ref Ray3 ray, ref Vector3 spherePosition, float radius, float maximumLength, out RayHit hit)
        {
            Vector3 normalizedDirection;
            float length = ray.Direction.Length();
            Vector3.Div(ref ray.Direction, length, out normalizedDirection);
            maximumLength *= length;
            hit = new RayHit();
            Vector3 m;
            Vector3.Sub(ref ray.Position, ref spherePosition, out m);
            float b = m.Dot(normalizedDirection);
            ...
            if (hit.T > maximumLength)
                return false;
            hit.T /= length;
            Vector3.Mul(ref normalizedDirection, hit.T, out hit.Location);
            Vector3.Add(ref hit.Location, ref ray.Position, out hit.Location);
            Vector3.Sub(ref hit.Location, ref spherePosition, out hit.Normal);
        ...
        public static void GetExpandedBoundingBox(ref ConvexShape shape, ref RigidTransform3 transform, ref Vector3 sweep, out BoundingBox3 boundingBox)
        {
        ...
        public static void ExpandBoundingBox(ref BoundingBox3 boundingBox, ref Vector3 sweep)
        {
            if (sweep.X > 0)
        ...
        public static void GetTriangleBoundingBox(ref Vector3 a, ref Vector3 b, ref Vector3 c, out BoundingBox3 aabb)
        {
            aabb = new BoundingBox3();
            //X axis
            if (a.X > b.X && a.X > c.X)
            {
                //A is max
        ...
        {
            Vector3 offset;
            Vector3.Sub(ref p, ref entity.position, out offset);
        ...
        internal static void UpdateOrientationRK4(ref Quaternion q, ref Matrix3 localInertiaTensorInverse, ref Vector3 angularMomentum, float dt, out Quaternion newOrientation)
        {
            ...
            Quaternion s2;
            Quaternion.Mul(ref d1, dt * .5f, out s2);
            ...
            Quaternion s3;
            Quaternion.Mul(ref d2, dt * .5f, out s3);
            ...
            Quaternion s4;
            Quaternion.Mul(ref d3, dt, out s4);
            ...
            Quaternion.Mul(ref d1, dt / 6, out d1);
            Quaternion.Mul(ref d2, dt / 3, out d2);
            Quaternion.Mul(ref d3, dt / 3, out d3);
            Quaternion.Mul(ref d4, dt / 6, out d4);
            Quaternion added;
        ...
        internal static void DifferentiateQuaternion(ref Quaternion orientation, ref Matrix3 localInertiaTensorInverse, ref Vector3 angularMomentum, out Quaternion orientationChange)
        {
            Quaternion normalizedOrientation;
            Quaternion.Normalize(ref orientation, out normalizedOrientation);
            Matrix3 tempRotMat;
            Matrix3.FromQuaternion(ref normalizedOrientation, out tempRotMat);
            Matrix3 tempInertiaTensorInverse;
            Matrix3.MultiplyTransposed(ref tempRotMat, ref localInertiaTensorInverse, out tempInertiaTensorInverse);
            Matrix3.Multiply(ref tempInertiaTensorInverse, ref tempRotMat, out tempInertiaTensorInverse);
            Vector3 halfspin;
            Vector3.Transform(ref angularMomentum, ref tempInertiaTensorInverse, out halfspin);
            Vector3.Mul(ref halfspin, .5f, out halfspin);
