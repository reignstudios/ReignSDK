...
namespace BEPUphysics.Constraints.SingleEntity
{
    ...
    public class SingleEntityLinearMotor : SingleEntityConstraint, I3DImpulseConstraintWithError
    {
        ...
        private Matrix3 effectiveMassMatrix;
        ...
        public Vector3 LocalPoint
        {
            ...
            {
                localPoint = value;
                Vector3.Transform(ref localPoint, ref entity.orientationMatrix, out worldPoint);
        ...
        public Vector3 Point
        {
            ...
            {
                worldPoint = value;
                Vector3.Sub(ref worldPoint, ref entity.position, out localPoint);
                Vector3.TransformTranspose(ref localPoint, ref entity.orientationMatrix, out localPoint);
            }
        }
        ...
        public Vector3 RelativeVelocity
        {
            get
            {
                ...
                Vector3.Sub(ref lambda, ref entity.linearVelocity, out lambda);
                return lambda;
            }
        }
        ...
        public override float SolveIteration()
        {
            ...
            Vector3.Mul(ref accumulatedImpulse, usedSoftness, out softnessVelocity);
            Vector3.Sub(ref lambda, ref softnessVelocity, out lambda);

            //In terms of an impulse (an instantaneous change in momentum), what is it?
            Vector3.Transform(ref lambda, ref effectiveMassMatrix, out lambda);
        ...
        public override void Update(float dt)
        {
            ...
            Vector3.Transform(ref localPoint, ref entity.orientationMatrix, out r);
            Vector3.Add(ref r, ref entity.position, out worldPoint);


            if (settings.mode == MotorMode.Servomechanism)
            {
                Vector3.Sub(ref settings.servo.goal, ref worldPoint, out error);
                ...
                if (separationDistance > Toolbox.BigEpsilon)
                {
                    ...
                    Vector3.Mul(ref error, correctionSpeed / separationDistance, out biasVelocity);
            ...
            Matrix3.FromScale(entity.inverseMass, out linearComponent);
            Matrix3 rACrossProduct;
            Matrix3.FromCross(ref r, out rACrossProduct);
            Matrix3 angularComponentA;
            Matrix3.Multiply(ref rACrossProduct, ref entity.inertiaTensorInverse, out angularComponentA);
            Matrix3.Multiply(ref angularComponentA, ref rACrossProduct, out angularComponentA);
            Matrix3.Sub(ref linearComponent, ref angularComponentA, out effectiveMassMatrix);

            effectiveMassMatrix.X.X += usedSoftness;
            effectiveMassMatrix.Y.Y += usedSoftness;
            effectiveMassMatrix.Z.Z += usedSoftness;

            Matrix3.Invert(ref effectiveMassMatrix, out effectiveMassMatrix);
