...
namespace BEPUphysics.Constraints.TwoEntity.Motors
{
    ...
    public class LinearAxisMotor : Motor, I1DImpulseConstraintWithError, I1DJacobianConstraint
    {
        ...
        public Vector3 Axis
        {
            ...
            set
            {
                worldAxis = value.Normalize();
                Vector3.TransformTranspose(ref worldAxis, ref connectionA.orientationMatrix, out localAxis);
            }
        }
        ...
        public Vector3 LocalAxis
        {
            ...
            set
            {
                localAxis = value.Normalize();
                Vector3.Transform(ref localAxis, ref connectionA.orientationMatrix, out worldAxis);
            }
        }
        ...
        public Vector3 OffsetA
        {
            ...
            set
            {
                worldOffsetA = value; 
                worldAnchorA = connectionA.position + worldOffsetA;
                Vector3.TransformTranspose(ref worldOffsetA, ref connectionA.orientationMatrix, out localAnchorA); //Looks weird, but localAnchorA is "localOffsetA."
        ...
        public Vector3 OffsetB
        {
            ...
            set
            {
                worldOffsetB = value;
                worldAnchorB = connectionB.position + worldOffsetB;
                Vector3.TransformTranspose(ref worldOffsetB, ref connectionB.orientationMatrix, out localAnchorB);//Looks weird, but localAnchorB is "localOffsetB."
            }
        }
        ...
        public override float SolveIteration()
        {
            ...
            accumulatedImpulse = MathUtilities.Clamp(accumulatedImpulse + lambda, -maxForceDt, maxForceDt);
            ...
            {
                Vector3.Mul(ref jLinearA, lambda, out impulse);
                connectionA.ApplyLinearImpulse(ref impulse);
                Vector3.Mul(ref jAngularA, lambda, out impulse);
                connectionA.ApplyAngularImpulse(ref impulse);
            }
            if (connectionB.isDynamic)
            {
                Vector3.Mul(ref jLinearB, lambda, out impulse);
                connectionB.ApplyLinearImpulse(ref impulse);
                Vector3.Mul(ref jAngularB, lambda, out impulse);
        ...
        public override void Update(float dt)
        {
            ...
            Vector3.Sub(ref worldAnchorB, ref connectionA.position, out rA);
            Vector3.Transform(ref localAxis, ref connectionA.orientationMatrix, out worldAxis);
            ...
            if (settings.mode == MotorMode.Servomechanism)
            {
                ...
                biasVelocity = MathUtilities.Clamp(biasVelocity, -settings.servo.maxCorrectiveVelocity, settings.servo.maxCorrectiveVelocity);
            }
            else
            {
            ...
            if (connectionA.isDynamic)
            {
                Vector3.Transform(ref jAngularA, ref connectionA.inertiaTensorInverse, out intermediate);
            ...
            if (connectionB.isDynamic)
            {
                Vector3.Transform(ref jAngularB, ref connectionB.inertiaTensorInverse, out intermediate);
        ...
        public override void ExclusiveUpdate()
        {
            ...
            if (connectionA.isDynamic)
            {
                Vector3.Mul(ref jLinearA, accumulatedImpulse, out impulse);
                connectionA.ApplyLinearImpulse(ref impulse);
                Vector3.Mul(ref jAngularA, accumulatedImpulse, out impulse);
                connectionA.ApplyAngularImpulse(ref impulse);
            }
            if (connectionB.isDynamic)
            {
                Vector3.Mul(ref jLinearB, accumulatedImpulse, out impulse);
                connectionB.ApplyLinearImpulse(ref impulse);
                Vector3.Mul(ref jAngularB, accumulatedImpulse, out impulse);
                connectionB.ApplyAngularImpulse(ref impulse);
            }
        }
    }
