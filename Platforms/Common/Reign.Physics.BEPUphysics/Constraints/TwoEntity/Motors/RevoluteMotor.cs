...
namespace BEPUphysics.Constraints.TwoEntity.Motors
{
    ...
    public class RevoluteMotor : Motor, I1DImpulseConstraintWithError, I1DJacobianConstraint
    {
        ...
        private float GetDistanceFromGoal(float angle)
        {
            ...
            float goalAngle = MathUtilities.WrapAngle(settings.servo.goal);
            if (goalAngle > 0)
            {
                if (angle > goalAngle)
                    forwardDistance = angle - goalAngle;
                else if (angle > 0)
                    forwardDistance = MathUtilities.Pi2 - goalAngle + angle;
                else //if (angle <= 0)
                    forwardDistance = MathUtilities.Pi2 - goalAngle + angle;
            }
            else
            ...
            return forwardDistance > MathUtilities.Pi ? MathUtilities.Pi2 - forwardDistance : -forwardDistance;
        }
    }
}