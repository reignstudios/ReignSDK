...
namespace BEPUphysics.Constraints.TwoEntity.JointLimits
{
    ...
    public class RevoluteLimit : JointLimit, I2DImpulseConstraintWithError, I2DJacobianConstraint
    {
        ...
        public Vector3 LocalTestAxis
        {
            ...
            set
            {
                localTestAxis = value.Normalize();
                Vector3.Transform(ref localTestAxis, ref connectionB.orientationMatrix, out worldTestAxis);
            }
        }
        ...
        public float MaximumAngle
        {
            ...
            set
            {
                maximumAngle = value % (MathUtilities.Pi2);
                if (minimumAngle > MathUtilities.Pi)
                    minimumAngle -= MathUtilities.Pi2;
                if (minimumAngle <= -MathUtilities.Pi)
                    minimumAngle += MathUtilities.Pi2;
            }
        }
        ...
        public float MinimumAngle
        {
            ...
            set
            {
                minimumAngle = value % (MathUtilities.Pi2);
                if (minimumAngle > MathUtilities.Pi)
                    minimumAngle -= MathUtilities.Pi2;
                if (minimumAngle <= -MathUtilities.Pi)
                    minimumAngle += MathUtilities.Pi2;
        ...
        public Vector3 TestAxis
        {
            ...
            set
            {
                worldTestAxis = value.Normalize();
                Vector3.TransformTranspose(ref worldTestAxis, ref connectionB.orientationMatrix, out localTestAxis);
            }
        }
        ...
        /// maximum plane.  The X.X field represents the minimum plane mass matrix
        /// and the Y.Y field represents the maximum plane mass matrix.
        /// </summary>
        /// <param name="massMatrix">Mass matrix of the constraint.</param>
        public void GetMassMatrix(out Matrix2 massMatrix)
        {
            massMatrix.X.X = velocityToImpulse.X;
            massMatrix.Y.Y = velocityToImpulse.Y;
            massMatrix.X.Y = 0;
            massMatrix.Y.X = 0;
			...
			massMatrix.Y.z = 0;
			massMatrix.Y.w = 0;
			massMatrix.z = Vector2.Zero;
			massMatrix.w = Vector2.Zero;
        }
        ...
        public override float SolveIteration()
        {
            ...
            if (minIsActive)
            {
                ...
                accumulatedImpulse.X = Math.Max(accumulatedImpulse.X + lambda, 0);
                ...
                {
                    Vector3.Mul(ref jacobianMinA, lambda, out impulse);
                    connectionA.ApplyAngularImpulse(ref impulse);
                }
                if (connectionB.isDynamic)
                {
                    Vector3.Mul(ref jacobianMinB, lambda, out impulse);
            ...
            if (maxIsActive)
            {
                ...
                accumulatedImpulse.Y = Math.Max(accumulatedImpulse.Y + lambda, 0);
                ...
                if (connectionA.isDynamic)
                {
                    Vector3.Mul(ref jacobianMaxA, lambda, out impulse);
                    connectionA.ApplyAngularImpulse(ref impulse);
                }
                if (connectionB.isDynamic)
                {
                    Vector3.Mul(ref jacobianMaxB, lambda, out impulse);
                    connectionB.ApplyAngularImpulse(ref impulse);
                }
        ...
        public override void Update(float dt)
        {
            ...
            Matrix3 rotation;
            Matrix3.FromRotationAxis(ref basis.primaryAxis, minimumAngle + MathUtilities.PiHalf, out rotation);
            Vector3.Transform(ref basis.xAxis, ref rotation, out minPlaneNormal);
            Matrix3.FromRotationAxis(ref basis.primaryAxis, maximumAngle - MathUtilities.PiHalf, out rotation);
            Vector3.Transform(ref basis.xAxis, ref rotation, out maxPlaneNormal);
            ...
            if (minIsActive)
            {
                ...
                jacobianMinA = jacobianMinA.Normalize();
            ...
            if (maxIsActive)
            {
                ...
                jacobianMaxA = jacobianMaxA.Normalize();
            ...
            if (minIsActive)
            {
                biasVelocity.X = Math.Min(Math.Max(0, error.X - margin) * errorReduction, maxCorrectiveVelocity);
                if (bounciness > 0)
                {
                    ...
                    if (-relativeVelocity > bounceVelocityThreshold)
                        biasVelocity.X = Math.Max(biasVelocity.X, -bounciness * relativeVelocity);
                }
            }
            if (maxIsActive)
            {
                biasVelocity.Y = Math.Min(Math.Max(0, error.Y - margin) * errorReduction, maxCorrectiveVelocity);
                if (bounciness > 0)
                {
                    //Find the velocity contribution from each connection
                    if (maxIsActive)
                    {
                        ...
                        relativeVelocity += dot;
                        if (-relativeVelocity > bounceVelocityThreshold)
                            biasVelocity.Y = Math.Max(biasVelocity.Y, -bounciness * relativeVelocity);
                    }
                }
            }
            ...
            if (connectionA.isDynamic)
            {
                if (minIsActive)
                {
                    Vector3.Transform(ref jacobianMinA, ref connectionA.inertiaTensorInverse, out transformedAxis);
                ...
                if (maxIsActive)
                {
                    Vector3.Transform(ref jacobianMaxA, ref connectionA.inertiaTensorInverse, out transformedAxis);
            ...
            if (connectionB.isDynamic)
            {
                if (minIsActive)
                {
                    Vector3.Transform(ref jacobianMinB, ref connectionB.inertiaTensorInverse, out transformedAxis);
                ...
                if (maxIsActive)
                {
                    Vector3.Transform(ref jacobianMaxB, ref connectionB.inertiaTensorInverse, out transformedAxis);
        ...
        public override void ExclusiveUpdate()
        {
            ...
            if (connectionA.isDynamic)
            {
                ...
                if (minIsActive)
                {
                    Vector3.Mul(ref jacobianMinA, accumulatedImpulse.X, out impulse);
                }
                if (maxIsActive)
                {
                    Vector3 temp;
                    Vector3.Mul(ref jacobianMaxA, accumulatedImpulse.Y, out temp);
            ...
            if (connectionB.isDynamic)
            {
                ...
                if (minIsActive)
                {
                    Vector3.Mul(ref jacobianMinB, accumulatedImpulse.X, out impulse);
                }
                if (maxIsActive)
                {
                    Vector3 temp;
                    Vector3.Mul(ref jacobianMaxB, accumulatedImpulse.Y, out temp);
                    Vector3.Add(ref impulse, ref temp, out impulse);
                }
                connectionB.ApplyAngularImpulse(ref impulse);
            }
        ...
        private float GetDistanceFromMinimum(float angle)
        {
            if (minimumAngle > 0)
            {
                ...
                if (angle > 0)
                    return MathUtilities.Pi2 - minimumAngle + angle;
                return MathUtilities.Pi2 - minimumAngle + angle;
            }
            if (angle < minimumAngle)
                return MathUtilities.Pi2 - minimumAngle + angle;
