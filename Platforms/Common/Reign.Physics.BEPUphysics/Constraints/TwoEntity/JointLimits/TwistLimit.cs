...
namespace BEPUphysics.Constraints.TwoEntity.JointLimits
{
    ...
    public class TwistLimit : JointLimit, I1DImpulseConstraintWithError, I1DJacobianConstraint
    {
        ...
        {
            worldTwistAxisA = worldTwistAxisA.Normalize();
            worldTwistAxisB = worldTwistAxisB.Normalize();
            ...
            worldXAxis = worldXAxis.Normalize();
        ...
        public override void Update(float dt)
        {
            ...
            //biasVelocity = MathUtilities.Clamp(-error * myCorrectionStrength / dt, -myMaxCorrectiveVelocity, myMaxCorrectiveVelocity);
            biasVelocity = Math.Min(Math.Max(0, Math.Abs(error) - margin) * errorReduction, maxCorrectiveVelocity);
            if (bounciness > 0)
            {
        ...
        {
            float errorFromMin = MathUtilities.Pi2 - distanceFromCurrent;
