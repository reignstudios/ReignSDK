...
namespace BEPUphysics.Constraints.TwoEntity.JointLimits
{
    ...
    public class SwingLimit : JointLimit, I1DImpulseConstraintWithError, I1DJacobianConstraint
    {
        ...
        public float MaximumAngle
        {
            ...
            set { minimumCosine = (float)Math.Cos(MathUtilities.Clamp(value, 0, MathUtilities.Pi)); }
        }
        ...
        public float RelativeVelocity
        {
            get
            {
                if (isLimitActive)
                {
                    ...
                    Vector3.Sub(ref connectionA.angularVelocity, ref connectionB.angularVelocity, out relativeVelocity);
                    float lambda;
        ...
        public override float SolveIteration()
        {
            ...
            //Apply the impulse
            Vector3 impulse;
            Vector3.Mul(ref hingeAxis, lambda, out impulse);
        ...
        public override void Update(float dt)
        {
            ...
            {
                //Vector3.Div(ref hingeAxis, (float)Math.Sqrt(lengthSquared), out hingeAxis);
            }
            else
            {
            ...
            if (connectionA.isDynamic)
            {
                Vector3.Transform(ref hingeAxis, ref connectionA.inertiaTensorInverse, out transformedAxis);
            ...
            if (connectionB.isDynamic)
            {
                Vector3.Transform(ref hingeAxis, ref connectionB.inertiaTensorInverse, out transformedAxis);
        ...
        public override void ExclusiveUpdate()
        {
            ...
            Vector3 impulse;
            Vector3.Mul(ref hingeAxis, accumulatedImpulse, out impulse);
            if (connectionA.isDynamic)
            {
                connectionA.ApplyAngularImpulse(ref impulse);
            }
