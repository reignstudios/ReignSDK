    ...
    public class TwistJoint : Joint, I1DImpulseConstraintWithError, I1DJacobianConstraint
    {
        ...
        public Vector3 LocalAxisB
        {
            ...
            set
            {
                ...
                Vector3.Transform(ref localAxisB, ref connectionA.orientationMatrix, out worldAxisB);
                Initialize();
            }
        }
        ...
        public override float SolveIteration()
        {
            ...
            {
                Vector3.Mul(ref jacobianA, lambda, out impulse);
                connectionA.ApplyAngularImpulse(ref impulse);
            }
            if (connectionB.isDynamic)
            {
                Vector3.Mul(ref jacobianB, lambda, out impulse);
        ...
        public override void Update(float dt)
        {
            ...
            Vector3 bAxisY;
            Vector3.Transform(ref localAxisA, ref connectionA.orientationMatrix, out worldAxisA);
            Vector3.Transform(ref aLocalAxisY, ref connectionA.orientationMatrix, out aAxisY);
            Vector3.Transform(ref aLocalAxisZ, ref connectionA.orientationMatrix, out aAxisZ);
            Vector3.Transform(ref localAxisB, ref connectionB.orientationMatrix, out worldAxisB);
            Vector3.Transform(ref bLocalAxisY, ref connectionB.orientationMatrix, out bAxisY);
            ...
            if (connectionA.isDynamic)
            {
                Vector3.Transform(ref jacobianA, ref connectionA.inertiaTensorInverse, out transformedAxis);
            ...
            if (connectionB.isDynamic)
            {
                Vector3.Transform(ref jacobianB, ref connectionB.inertiaTensorInverse, out transformedAxis);
        ...
        public override void ExclusiveUpdate()
        {
            ...
            if (connectionA.isDynamic)
            {
                Vector3.Mul(ref jacobianA, accumulatedImpulse, out impulse);
                connectionA.ApplyAngularImpulse(ref impulse);
            }
            if (connectionB.isDynamic)
            {
                Vector3.Mul(ref jacobianB, accumulatedImpulse, out impulse);
                connectionB.ApplyAngularImpulse(ref impulse);
            }
        }
