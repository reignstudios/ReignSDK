...
namespace BEPUphysics.Vehicle
{
    ...
    public class RaycastWheelShape : WheelShape
    {
        ...
        public override void UpdateWorldTransform()
        {
            ...
            worldTransform = Matrix4.FromMatrix3(wheel.vehicle.Body.BufferedStates.InterpolatedStates.OrientationMatrix);
            ...
            Vector3 localSpinAxis = wheel.localForwardDirection.Cross(wheel.suspension.localDirection);
            Matrix4.FromRotationAxis(ref localSpinAxis, spinAngle, out spinTransform);


            Matrix4 localTurnTransform;
            Matrix4.Multiply(ref localGraphicTransform, ref spinTransform, out localTurnTransform);
            Matrix4.Multiply(ref localTurnTransform, ref steeringTransform, out localTurnTransform);
            //Matrix4.Multiply(ref localTurnTransform, ref spinTransform, out localTurnTransform);
            Matrix4.Multiply(ref localTurnTransform, ref worldTransform, out worldTransform);
        ...
        protected internal override bool FindSupport(out Vector3 location, out Vector3 normal, out float suspensionLength, out Collidable supportingCollidable, out Entity entity, out Material material)
        {
            ...
            for (int i = 0; i < detector.CollisionInformation.pairs.Count; i++)
            {
                ...
                if (testCollidable != null)
                {
                    if (CollisionRules.CollisionRuleCalculator(this, testCollidable) == CollisionRule.Normal &&
                        testCollidable.RayCast(new Ray3(wheel.suspension.worldAttachmentPoint, wheel.suspension.worldDirection), wheel.suspension.restLength, out rayHit) &&
            ...
            if (hit)
            {
                if (suspensionLength > 0)
                    ...
                    Vector3.Neg(ref wheel.suspension.worldDirection, out normal);
                return true;
            }
            return false;
        }
        ...
        protected internal override void UpdateDetectorPosition()
        {
            ...
            Vector3.Sub(ref newPosition, ref wheel.vehicle.Body.position, out linearVelocity);
