...
namespace BEPUphysics.Vehicle
{
    ...
    public class WheelDrivingMotor : ISolverSettings
    {
        ...
        internal float ApplyImpulse()
        {
            ...
            if (targetSpeed > 0)
                accumulatedImpulse = MathUtilities.Clamp(accumulatedImpulse, 0, maxMotorForceDt); //Math.Min(Math.Max(accumulatedImpulse, 0), myMaxMotorForceDt);
            else if (targetSpeed < 0)
                accumulatedImpulse = MathUtilities.Clamp(accumulatedImpulse, maxMotorForceDt, 0); //Math.Max(Math.Min(accumulatedImpulse, 0), myMaxMotorForceDt);
            else
                accumulatedImpulse = 0;
            //Friction
            float maxForce = currentFrictionCoefficient * wheel.suspension.accumulatedImpulse;
            accumulatedImpulse = MathUtilities.Clamp(accumulatedImpulse, maxForce, -maxForce);
