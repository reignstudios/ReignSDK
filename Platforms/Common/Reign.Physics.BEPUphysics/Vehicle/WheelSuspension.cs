    ...
    public class WheelSuspension : ISpringSettings, ISolverSettings
    {
        ...
        public Vector3 LocalAttachmentPoint
        {
            ...
            set
            {
                ...
                {
                    Vector3.Transform(ref localAttachmentPoint, ref wheel.vehicle.Body.CollisionInformation.worldTransform, out worldAttachmentPoint);
                }
                else
        ...
        public Vector3 LocalDirection
        {
            ...
            set
            {
                localDirection = value.Normalize();
                if (wheel != null && wheel.vehicle != null)
                    Vector3.Transform(ref localDirection, ref wheel.vehicle.Body.orientationMatrix, out worldDirection);
                else
        ...
        public Vector3 WorldAttachmentPoint
        {
            ...
            set
            {
                ...
                if (wheel != null && wheel.vehicle != null)
                {
                    Vector3.TransformInversed(ref worldAttachmentPoint, ref wheel.vehicle.Body.CollisionInformation.worldTransform, out localAttachmentPoint);
                }
                else
        ...
        public Vector3 WorldDirection
        {
            ...
            set
            {
                worldDirection = value.Normalize();
                if (wheel != null && wheel.vehicle != null)
                {
                    Vector3.TransformTranspose(ref worldDirection, ref wheel.Vehicle.Body.orientationMatrix, out localDirection);
                }
                else
        ...
        internal float ApplyImpulse()
        {
            ...
            accumulatedImpulse = MathUtilities.Clamp(accumulatedImpulse + lambda, -maximumSpringForce, 0);
        ...
        internal void PreStep(float dt)
        {
            ...
            if (vehicleEntity.isDynamic)
            {
                tX = angularAX * vehicleEntity.inertiaTensorInverse.X.X + angularAY * vehicleEntity.inertiaTensorInverse.Y.X + angularAZ * vehicleEntity.inertiaTensorInverse.Z.X;
                tY = angularAX * vehicleEntity.inertiaTensorInverse.X.Y + angularAY * vehicleEntity.inertiaTensorInverse.Y.Y + angularAZ * vehicleEntity.inertiaTensorInverse.Z.Y;
                tZ = angularAX * vehicleEntity.inertiaTensorInverse.X.Z + angularAY * vehicleEntity.inertiaTensorInverse.Y.Z + angularAZ * vehicleEntity.inertiaTensorInverse.Z.Z;
            ...
            if (supportIsDynamic)
            {
                tX = angularBX * supportEntity.inertiaTensorInverse.X.X + angularBY * supportEntity.inertiaTensorInverse.Y.X + angularBZ * supportEntity.inertiaTensorInverse.Z.X;
                tY = angularBX * supportEntity.inertiaTensorInverse.X.Y + angularBY * supportEntity.inertiaTensorInverse.Y.Y + angularBZ * supportEntity.inertiaTensorInverse.Z.Y;
                tZ = angularBX * supportEntity.inertiaTensorInverse.X.Z + angularBY * supportEntity.inertiaTensorInverse.Y.Z + angularBZ * supportEntity.inertiaTensorInverse.Z.Z;
