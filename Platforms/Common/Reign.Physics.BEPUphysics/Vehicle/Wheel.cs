...
namespace BEPUphysics.Vehicle
{
    ...
    public class Wheel
    {
        ...
        public Vector3 LocalForwardDirection
        {
            ...
            set
            {
                localForwardDirection = value.Normalize();
                if (vehicle != null)
                    Vector3.Transform(ref localForwardDirection, ref Vehicle.Body.orientationMatrix, out worldForwardDirection);
                else
        ...
        public Vector3 WorldForwardDirection
        {
            ...
            set
            {
                worldForwardDirection = value.Normalize();
                if (vehicle != null)
                {
        ...
        {
            Matrix4.FromRotationAxis(ref suspension.localDirection, shape.steeringAngle, out shape.steeringTransform);
            Vector3.TransformNormal(ref localForwardDirection, ref shape.steeringTransform, out worldForwardDirection);
            Vector3.Transform(ref worldForwardDirection, ref Vehicle.Body.orientationMatrix, out worldForwardDirection);
            if (HasSupport)
            {
                Vector3.Sub(ref supportLocation, ref Vehicle.Body.position, out ra);
                if (supportingEntity != null)
                    Vector3.Sub(ref supportLocation, ref SupportingEntity.position, out rb);
