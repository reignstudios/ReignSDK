...
namespace BEPUphysics.CollisionShapes
{
    ...
    public struct CompoundShapeEntry
    {
        ...
        ///</summary>
        public RigidTransform3 LocalTransform;
        /// <summary>
        ...
        public CompoundShapeEntry(EntityShape shape, RigidTransform3 localTransform, float weight)
        {
        ...
        {
            LocalTransform = new RigidTransform3(position);
            Shape = shape;
            Weight = weight;
        }
        ...
        {
            LocalTransform = new RigidTransform3(orientation);
            Shape = shape;
            Weight = weight;
        }
        ///<summary>
        ...
        {
            LocalTransform = RigidTransform3.Identity;
            Shape = shape;
            Weight = weight;
        }
        ...
        ///<param name="shape">Shape to use.</param>
        ///<param name="localTransform">Local transform of the shape.</param>
        public CompoundShapeEntry(EntityShape shape, RigidTransform3 localTransform)
        {
            LocalTransform = localTransform;
            Shape = shape;
    ...
    public class CompoundShape : EntityShape
    {
        ...
        public override Vector3 ComputeCenter()
        {
            ...
            for (int i = 0; i < shapes.count; i++)
            {
                ...
                Vector3.Mul(ref shapes.Elements[i].LocalTransform.Position, shapes.Elements[i].Weight, out centerContribution);
                Vector3.Add(ref center, ref centerContribution, out center);

            }
            Vector3.Mul(ref center, 1 / totalWeight, out center);
            return center;
        }
        ...
        public static Vector3 ComputeCenter(IList<CompoundChildData> childData)
        {
            ...
            Vector3.Div(ref center, volume, out center);
            return center;
        ...
        {
            var volumeDistribution = new Matrix3();
            float totalWeight = 0;
            for (int i = 0; i < shapes.count; i++)
            {
                totalWeight += shapes.Elements[i].Weight;
                ...
                Matrix3.Add(ref contribution, ref volumeDistribution, out volumeDistribution);

            }
            Matrix3.Mul(ref volumeDistribution, 1 / totalWeight, out volumeDistribution);
        ...
        /// <returns>Volume distribution of the shape.</returns>
        public static Matrix3 ComputeVolumeDistribution(IList<CompoundShapeEntry> entries, out Vector3 center)
        {
            center = new Vector3();
            float totalWeight = 0;
            ...
            for (int i = 0; i < entries.Count; i++)
            {
                RigidTransform3 transform = entries[i].LocalTransform;
                Matrix3 contribution;
                GetContribution(entries[i].Shape, ref transform, ref center, entries[i].Weight, out contribution);
                Matrix3.Add(ref volumeDistribution, ref contribution, out volumeDistribution);
            }
            return volumeDistribution;
        }
        ...
        public static void GetContribution(EntityShape shape, ref RigidTransform3 transform, ref Vector3 center, float weight, out Matrix3 contribution)
        {
        ...
        public static void TransformContribution(ref RigidTransform3 transform, ref Vector3 center, ref Matrix3 baseContribution, float weight, out Matrix3 contribution)
        {
            Matrix3 rotation;
            Matrix3.FromQuaternion(ref transform.Orientation, out rotation);
            Matrix3 temp;

            //TODO: Verify contribution

            //Do angular transformed contribution first...
            Matrix3.MultiplyTransposed(ref rotation, ref baseContribution, out temp);
            Matrix3.Multiply(ref temp, ref rotation, out temp);

            contribution = temp;

            //Now add in the offset from the origin.
            Vector3 offset;
            Vector3.Sub(ref transform.Position, ref center, out offset);
            Matrix3 innerProduct;
            Matrix3.FromScale(offset.LengthSquared(), out innerProduct);
            Matrix3 outerProduct;
            Matrix3.FromOuterProduct(ref offset, ref offset, out outerProduct);

            Matrix3.Sub(ref innerProduct, ref outerProduct, out temp);

            Matrix3.Add(ref contribution, ref temp, out contribution);
