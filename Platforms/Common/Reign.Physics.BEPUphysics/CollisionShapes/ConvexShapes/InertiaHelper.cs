...
namespace BEPUphysics.CollisionShapes.ConvexShapes
{
    ...
    public class InertiaHelper
    {
        ...
        public static Matrix3 ComputeVolumeDistribution(ConvexShape shape, out float volume)
        {
            var pointContributions = Resources.GetVectorList();
            GetPoints(shape, out volume, pointContributions);
            Vector3 center = AveragePoints(pointContributions);
            Matrix3 volumeDistribution = ComputeVolumeDistribution(pointContributions, ref center);
            Resources.GiveBack(pointContributions);
        ...
        ///<param name="volume">Volume of the shape.</param>
        ///<returns>Volume distribution of the shape.</returns>
        public static Matrix3 ComputeVolumeDistribution(ConvexShape shape, ref Vector3 center, out float volume)
        {
            var pointContributions = Resources.GetVectorList();
            GetPoints(shape, out volume, pointContributions);
            Matrix3 volumeDistribution = ComputeVolumeDistribution(pointContributions, ref center);
        ...
        public static Matrix3 ComputeVolumeDistribution(RawList<Vector3> pointContributions, ref Vector3 center)
        {
        ...
        public static void GetPoints(ConvexShape shape, out float volume, RawList<Vector3> outputPointContributions)
        {
            ...
            for (int i = 0; i < NumberOfSamplesPerDimension; i++)
            {
                ...
                {
                    //Ray3 cast from one direction.  If it succeeds, try the other way.  This forms an interval in which inertia tensor contributions are contained.
                    RayHit hit;
                    if (shape.RayTest(ref ray, ref transform, maxLength, out hit))
                    {
                        Vector3.Mul(ref ray.Direction, maxLength, out oppositeRay.Position);
                        Vector3.Add(ref oppositeRay.Position, ref ray.Position, out oppositeRay.Position);
                        Vector3.Neg(ref ray.Direction, out oppositeRay.Direction);
                        RayHit oppositeHit;
                ...
                Vector3 subtract;
                Vector3.Mul(ref increment2, NumberOfSamplesPerDimension, out subtract);
                Vector3.Sub(ref ray.Position, ref subtract, out ray.Position);
            }
        ...
        private static void ScanObject(float rayIncrement, float maxLength, ref Vector3 increment1, ref Vector3 increment2, ref Ray3 ray, ref RayHit startHit, ref RayHit endHit, RawList<Vector3> pointContributions, out float volume)
        {
            Vector3 cell;
            Vector3.Mul(ref ray.Direction, rayIncrement, out cell);
            ...
            {
                Vector3 position;
                Vector3.Mul(ref ray.Direction, (i + .5f) * rayIncrement, out position);
        ...
        public static void GetPointContribution(float pointWeight, ref Vector3 center, Vector3 p, out Matrix3 contribution)
        {
            Vector3.Sub(ref p, ref center, out p);
            float xx = pointWeight * p.X * p.X;
            float yy = pointWeight * p.Y * p.Y;
            float zz = pointWeight * p.Z * p.Z;
            contribution.X.X = yy + zz;
            contribution.Y.Y = xx + zz;
            contribution.Z.Z = xx + yy;
            contribution.X.Y = -pointWeight * p.X * p.Y;
            contribution.X.Z = -pointWeight * p.X * p.Z;
            contribution.Y.Z = -pointWeight * p.Y * p.Z;
            contribution.Y.X = contribution.X.Y;
            contribution.Z.X = contribution.X.Z;
            contribution.Z.Y = contribution.Y.Z;
			
			contribution.X.w = 0;
			contribution.Y.w = 0;
			contribution.Z.w = 0;
			contribution.w = Vector3.Zero;
        }
    }
}
