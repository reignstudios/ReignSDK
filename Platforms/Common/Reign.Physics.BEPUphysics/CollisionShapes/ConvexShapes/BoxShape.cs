...
namespace BEPUphysics.CollisionShapes.ConvexShapes
{
    ...
    public class BoxShape : ConvexShape
    {
        ...
        public override void GetBoundingBox(ref RigidTransform3 shapeTransform, out BoundingBox3 boundingBox)
        {
            ...
            var right = new Vector3(Math.Sign(o.X.X) * halfWidth, Math.Sign(o.Y.X) * halfHeight, Math.Sign(o.Z.X) * halfLength);

            var up = new Vector3(Math.Sign(o.X.Y) * halfWidth, Math.Sign(o.Y.Y) * halfHeight, Math.Sign(o.Z.Y) * halfLength);

            var backward = new Vector3(Math.Sign(o.X.Z) * halfWidth, Math.Sign(o.Y.Z) * halfHeight, Math.Sign(o.Z.Z) * halfLength);
        ...
        public override Matrix3 ComputeVolumeDistribution(out float volume)
        {
            ...
            volumeDistribution.X.X = (halfHeightSquared + halfLengthSquared) * inv3;
            volumeDistribution.Y.Y = (halfWidthSquared + halfLengthSquared) * inv3;
            volumeDistribution.Z.Z = (halfWidthSquared + halfHeightSquared) * inv3;
        ...
        /// </summary>
        /// <param name="ray">Ray3 to test against the box.</param>
        ...
        public override bool RayTest(ref Ray3 ray, ref RigidTransform3 transform, float maximumLength, out RayHit hit)
        {
            ...
            Vector3 localOrigin;
            Vector3.Sub(ref ray.Position, ref transform.Position, out localOrigin);
            ...
            if (tmin > tmax)
                return false;
            hit.T = tmin;
            Vector3.Mul(ref ray.Direction, tmin, out hit.Location);
