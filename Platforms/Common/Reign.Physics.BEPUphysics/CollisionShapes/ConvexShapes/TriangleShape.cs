...
namespace BEPUphysics.CollisionShapes.ConvexShapes
{
    ...
    public class TriangleShape : ConvexShape
    {
        ...
        public override Matrix3 ComputeVolumeDistribution(out float volume)
        {
            ...
            //Sub the position from the distribution, moving into a 'body space' relative to itself.
            //        [ (j * j + z * z)  (-j * j)  (-j * z) ]
            //I = I + [ (-j * j)  (j * j + z * z)  (-j * z) ]
            //	      [ (-j * z)  (-j * z)  (j * j + j * j) ]
            ...
            //localInertiaTensor += new Matrix4(j * j + k * k, -j * j, -j * k, 0, -j * j, j * j + k * k, -j * k, 0, -j * k, -j * k, j * j + j * j, 0, 0, 0, 0, 0); //No mass per point.
            var volumeDistribution = new Matrix3
			(
				new Vector3(massPerPoint * (j * j + k * k), massPerPoint * (-i * j), massPerPoint * (-i * k)),
                new Vector3(massPerPoint * (-i * j), massPerPoint * (i * i + k * k), massPerPoint * (-j * k)),
                new Vector3(massPerPoint * (-i * k), massPerPoint * (-j * k), massPerPoint * (i * i + j * j))
			);

            i = vB.X - center.X;
            j = vB.Y - center.Y;
            k = vB.Z - center.Z;
            var pointContribution = new Matrix3
			(
				new Vector3(massPerPoint * (j * j + k * k), massPerPoint * (-i * j), massPerPoint * (-i * k)),
                new Vector3(massPerPoint * (-i * j), massPerPoint * (i * i + k * k), massPerPoint * (-j * k)),
                new Vector3(massPerPoint * (-i * k), massPerPoint * (-j * k), massPerPoint * (i * i + j * j))
			);
            Matrix3.Add(ref volumeDistribution, ref pointContribution, out volumeDistribution);

            i = vC.X - center.X;
            j = vC.Y - center.Y;
            k = vC.Z - center.Z;
            pointContribution = new Matrix3
			(
				new Vector3(massPerPoint * (j * j + k * k), massPerPoint * (-i * j), massPerPoint * (-i * k)),
                new Vector3(massPerPoint * (-i * j), massPerPoint * (i * i + k * k), massPerPoint * (-j * k)),
                new Vector3(massPerPoint * (-i * k), massPerPoint * (-j * k), massPerPoint * (i * i + j * j))
        ...
        {
            return (vB - vA).Cross(vC - vA).Length() * collisionMargin;
        }
        ...
        public Vector3 GetNormal(RigidTransform3 transform)
        {
        ...
        /// </summary>
        /// <param name="ray">Ray3 to test against the triangle.</param>
        ...
        public override bool RayTest(ref Ray3 ray, ref RigidTransform3 transform, float maximumLength, out RayHit hit)
        {
            Matrix3 orientation;
            Matrix3.FromQuaternion(ref transform.Orientation, out orientation);
            Ray3 localRay;
            Quaternion conjugate;
            Quaternion.Conjugate(ref transform.Orientation, out conjugate);
            Vector3.Transform(ref ray.Direction, ref conjugate, out localRay.Direction);
            Vector3.Sub(ref ray.Position, ref transform.Position, out localRay.Position);
