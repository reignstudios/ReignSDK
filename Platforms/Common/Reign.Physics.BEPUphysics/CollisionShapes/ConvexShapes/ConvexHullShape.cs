...
namespace BEPUphysics.CollisionShapes.ConvexShapes
{
    ...
    public class ConvexHullShape : ConvexShape
    {
        ...
        public override void GetBoundingBox(ref RigidTransform3 shapeTransform, out BoundingBox3 boundingBox)
        {
            ...
            float minY, maxY;
            float minZ, maxZ;
            var right = new Vector3(o.X.X, o.Y.X, o.Z.X);
            var up = new Vector3(o.X.Y, o.Y.Y, o.Z.Y);
            var backward = new Vector3(o.X.Z, o.Y.Z, o.Z.Z);
            ...
            Vector3.Transform(ref vertices.Elements[minXIndex], ref o, out minXpoint);
            Vector3.Transform(ref vertices.Elements[maxXIndex], ref o, out maxXpoint);
            Vector3.Transform(ref vertices.Elements[minYIndex], ref o, out minYpoint);
            Vector3.Transform(ref vertices.Elements[maxYIndex], ref o, out maxYpoint);
            Vector3.Transform(ref vertices.Elements[minZIndex], ref o, out minZpoint);
            Vector3.Transform(ref vertices.Elements[maxZIndex], ref o, out maxZpoint);
        ...
        public static Vector3 ComputeCenter(IList<Vector3> vertices, out float volume, IList<int> outputSurfaceTriangles, IList<Vector3> outputLocalSurfaceVertices)
        {
            ...
            {
				var a = vertices[outputSurfaceTriangles[k + 1]] - vertices[outputSurfaceTriangles[k]];
				var b = vertices[outputSurfaceTriangles[k + 2]] - vertices[outputSurfaceTriangles[k]];
                volumes.Add(a.Cross(b).Dot(centroid - vertices[outputSurfaceTriangles[k]]));
                volume += volumes[k / 3];
        ...
        public override Matrix3 ComputeVolumeDistribution(out float volume)
        {
            ...
            Matrix3 toReturn = ComputeVolumeDistribution(volume, surfaceTriangles);
            Resources.GiveBack(surfaceTriangles);
            Resources.GiveBack(surfaceVertices);
            return toReturn;
        }
        ...
        public Matrix3 ComputeVolumeDistribution(float volume, IList<int> localSurfaceTriangles)
        {
            ...
            for (int i = 0; i < localSurfaceTriangles.Count; i += 3)
            {
                ...
                /*subInertiaTensor = new Matrix4(a, bo, co, 0,
                                              bo, b, ao, 0,
                                              co, ao, c, 0,
                                              0, 0, 0, 0);

                localInertiaTensor += subInertiaTensor;// +(offset.LengthSquared() * Matrix4.Identity - Toolbox.getOuterProduct(offset, offset));// *(determinant * density / 6);*/
            }
            a *= diagonalFactor;
            b *= diagonalFactor;
            c *= diagonalFactor;
        ...
        public override float ComputeMinimumRadius()
        {
            ...
            Vector3.Sub(ref b, ref c, out cb);
            Vector3.Sub(ref c, ref a, out ac);
            Vector3.Sub(ref d, ref a, out ad);
            Vector3.Sub(ref d, ref c, out cd);
