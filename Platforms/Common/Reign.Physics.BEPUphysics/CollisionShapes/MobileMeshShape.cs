...
namespace BEPUphysics.CollisionShapes
{
    ...
    public class MobileMeshShape : EntityShape
    {
        ...
        /// </summary>
        public AffineTransform3 Transform
        {
            get
            {
        ...
        public MobileMeshShape(Vector3[] vertices, int[] indices, AffineTransform3 localTransform, MobileMeshSolidity solidity)
        {
            ...
            {
                Vector3.Sub(ref surfaceVertices.Elements[i], ref distributionInfo.Center, out surfaceVertices.Elements[i]);
            }
        ...
        public MobileMeshShape(Vector3[] vertices, int[] indices, AffineTransform3 localTransform, MobileMeshSolidity solidity, out ShapeDistributionInformation distributionInfo)
        {
            this.solidity = solidity;
            var data = new TransformableMeshData(vertices, indices, localTransform);
            ComputeShapeInformation(data, out distributionInfo);
        ...
        /// </summary>
        /// <param name="ray">Ray3 in the local space of the shape to test.</param>
        /// <param name="hit">The first hit against the mesh, if any.</param>
        /// <returns>Whether or not the ray origin was in the mesh.</returns>
        public bool IsLocalRayOriginInMesh(ref Ray3 ray, out RayHit hit)
        {
        ...
        void ComputeSolidSidedness()
        {
            ...
            //}
            //Vector3.Div(ref alternateRay.Direction, (float)Math.Sqrt(lengthSquared), out alternateRay.Direction);
        ...
        TriangleSidedness ComputeSolidSidednessHelper(Ray3 ray)
        {
        ...
        void ComputeShapeInformation(TransformableMeshData data, out ShapeDistributionInformation shapeInformation)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < surfaceVertices.count; i++)
                {
                    Vector3.Transform(ref surfaceVertices.Elements[i], ref data.worldTransform, out surfaceVertices.Elements[i]);
                }
            }
            catch
            {
            ...
            if (solidity == MobileMeshSolidity.Solid)
            {
                ...
                bo *= offFactor;
                co *= offFactor;
                shapeInformation.VolumeDistribution = new Matrix3
				(
					new Vector3(a, bo, co),
                    new Vector3(bo, b, ao),
                    new Vector3(co, ao, c)
            ...
            else
            {
                ...
                { //Configure the inertia tensor to be local.
                    ...
                    Vector3 vAvB;
                    Vector3 vAvC;
                    Vector3.Sub(ref vB, ref vA, out vAvB);
                    Vector3.Sub(ref vC, ref vA, out vAvC);
                    Vector3 cross;
                ...
                { //Configure the inertia tensor to be local.
                    ...
                    Matrix3 innerProduct;
                    Matrix3.FromScale(vA.LengthSquared(), out innerProduct);
                    Matrix3 outerProduct;
                    Matrix3.FromOuterProduct(ref vA, ref vA, out outerProduct);
                    Matrix3 contribution;
                    Matrix3.Sub(ref innerProduct, ref outerProduct, out contribution);
                    Matrix3.Mul(ref contribution, weight, out contribution);
                    Matrix3.Add(ref shapeInformation.VolumeDistribution, ref contribution, out shapeInformation.VolumeDistribution);

                    Matrix3.FromScale(vB.LengthSquared(), out innerProduct);
                    Matrix3.FromOuterProduct(ref vB, ref vB, out outerProduct);
                    Matrix3.Sub(ref innerProduct, ref outerProduct, out outerProduct);
                    Matrix3.Mul(ref contribution, weight, out contribution);
                    Matrix3.Add(ref shapeInformation.VolumeDistribution, ref contribution, out shapeInformation.VolumeDistribution);

                    Matrix3.FromScale(vC.LengthSquared(), out innerProduct);
                    Matrix3.FromOuterProduct(ref vC, ref vC, out outerProduct);
                    Matrix3.Sub(ref innerProduct, ref outerProduct, out contribution);
                    Matrix3.Mul(ref contribution, weight, out contribution);
                    Matrix3.Add(ref shapeInformation.VolumeDistribution, ref contribution, out shapeInformation.VolumeDistribution);

                }
                Matrix3.Mul(ref shapeInformation.VolumeDistribution, 1 / (6 * totalWeight), out shapeInformation.VolumeDistribution);
            }

            ////Configure the inertia tensor to be local.
            //Vector3 finalOffset = shapeInformation.Center;
            //Matrix3 finalInnerProduct;
            //Matrix3.FromScale(finalOffset.LengthSquared(), out finalInnerProduct);
            //Matrix3 finalOuterProduct;
            //Matrix3.FromOuterProduct(ref finalOffset, ref finalOffset, out finalOuterProduct);

            //Matrix3 finalContribution;
            //Matrix3.Sub(ref finalInnerProduct, ref finalOuterProduct, out finalContribution);

            //Matrix3.Sub(ref shapeInformation.VolumeDistribution, ref finalContribution, out shapeInformation.VolumeDistribution);
        }
        ...
        //        //Sub the component of v which is parallel to the normal.
        ...
        private void GetBoundingBox(ref Matrix3 o, out BoundingBox3 boundingBox)
        {
            boundingBox = new BoundingBox3();
            //Sample the local directions from the matrix, implicitly transposed.
            var rightDirection = new Vector3(o.X.X, o.Y.X, o.Z.X);
            var upDirection = new Vector3(o.X.Y, o.Y.Y, o.Z.Y);
            var backDirection = new Vector3(o.X.Z, o.Y.Z, o.Z.Z);
            ...
            Vector3.Mul(ref rightDirection, meshCollisionMargin / (float)Math.Sqrt(rightDirection.Length()), out rightDirection);
            Vector3.Mul(ref upDirection, meshCollisionMargin / (float)Math.Sqrt(upDirection.Length()), out upDirection);
            Vector3.Mul(ref backDirection, meshCollisionMargin / (float)Math.Sqrt(backDirection.Length()), out backDirection);
            ...
            Vector3.Sub(ref leftElement, ref rightDirection, out leftElement);
            Vector3.Add(ref upElement, ref upDirection, out upElement);
            Vector3.Sub(ref downElement, ref upDirection, out downElement);
            Vector3.Add(ref backwardElement, ref backDirection, out backwardElement);
            Vector3.Sub(ref forwardElement, ref backDirection, out forwardElement);
            ...
            Vector3.Transform(ref rightElement, ref o, out vMaxX);
            Vector3.Transform(ref leftElement, ref o, out vMinX);
            Vector3.Transform(ref upElement, ref o, out vMaxY);
            Vector3.Transform(ref downElement, ref o, out vMinY);
            Vector3.Transform(ref backwardElement, ref o, out vMaxZ);
            Vector3.Transform(ref forwardElement, ref o, out vMinZ);
        ...
        public void GetBoundingBox(ref RigidTransform3 shapeTransform, out BoundingBox3 boundingBox)
        {
            ...
            //Matrix3 o;
            //Matrix3.FromQuaternion(ref shapeTransform.Orientation, out o);
            ////Sample the local directions from the orientation matrix, implicitly transposed.
            //Vector3 right = new Vector3(o.X.X * 100000, o.Y.X * 100000, o.Z.X * 100000);
            //Vector3 up = new Vector3(o.X.Y * 100000, o.Y.Y * 100000, o.Z.Y * 100000);
            //Vector3 backward = new Vector3(o.X.Z * 100000, o.Y.Z * 100000, o.Z.Z * 100000);
            //Vector3 left, down, forward;
            //Vector3.Neg(ref right, out left);
            //Vector3.Neg(ref up, out down);
            //Vector3.Neg(ref backward, out forward);
            //for (int i = 0; i < extents.count; i++)
            //{
            ...
            //Vector3.Transform(ref right, ref o, out right);
            //Vector3.Transform(ref left, ref o, out left);
            //Vector3.Transform(ref down, ref o, out down);
            //Vector3.Transform(ref up, ref o, out up);
            //Vector3.Transform(ref forward, ref o, out forward);
            //Vector3.Transform(ref backward, ref o, out backward);
        ...
        public void GetLocalBoundingBox(ref RigidTransform3 shapeTransform, ref AffineTransform3 spaceTransform, out BoundingBox3 boundingBox)
        {
            boundingBox = new BoundingBox3();
            ...
            AffineTransform3.Invert(ref spaceTransform, out transform);
            AffineTransform3.Multiply(ref shapeTransform, ref transform, out transform);

            GetBoundingBox(ref transform.Transform, out boundingBox);
        ...
        /// <param name="boundingBox">Bounding box in the local space.</param>
        public void GetSweptLocalBoundingBox(ref RigidTransform3 shapeTransform, ref AffineTransform3 spaceTransform, ref Vector3 sweep, out BoundingBox3 boundingBox)
        {
            GetLocalBoundingBox(ref shapeTransform, ref spaceTransform, out boundingBox);
            Vector3 expansion;
            Vector3.TransformTranspose(ref sweep, ref spaceTransform.Transform, out expansion);
