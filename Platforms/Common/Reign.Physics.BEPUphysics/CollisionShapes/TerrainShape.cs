...
namespace BEPUphysics.CollisionShapes
{
    ...
    public class TerrainShape : CollisionShape
    {
        ...
        public void GetBoundingBox(ref AffineTransform3 transform, out BoundingBox3 boundingBox)
        {
            ...
            for (int i = 0; i < heights.GetLength(0); i++)
            {
                for (int j = 0; j < heights.GetLength(1); j++)
                {
                    ...
                    Vector3.Transform(ref vertex, ref transform.Transform, out vertex);
                    if (vertex.X < minX)
                    {
                        minX = vertex.X;
                        minXvertex = vertex;
        ...
        public bool RayCast(ref Ray3 ray, float maximumLength, ref AffineTransform3 transform, out RayHit hit)
        {
        ...
        ///<param name="hit">Hit data of the ray cast, if any.</param>
        ///<returns>Whether or not the ray hit the transformed terrain shape.</returns>
        public bool RayCast(ref Ray3 ray, float maximumLength, ref AffineTransform3 transform, TriangleSidedness sidedness, out RayHit hit)
        {
            hit = new RayHit();
            //Put the ray into local space.
            Ray3 localRay;
            AffineTransform3 inverse;
            AffineTransform3.Invert(ref transform, out inverse);
            ...
            if (progressingOrigin.X < 0)
            {
                if (localRay.Direction.X > 0)
                {
                    ...
                    distance += timeToMinX;
                    Vector3 increment;
                    Vector3.Mul(ref localRay.Direction, timeToMinX, out increment);
            ...
            if (progressingOrigin.Z < 0)
            {
                if (localRay.Direction.Z > 0)
                {
                    ...
                    distance += timeToMinZ;
                    Vector3 increment;
                    Vector3.Mul(ref localRay.Direction, timeToMinZ, out increment);
                    Vector3.Add(ref increment, ref progressingOrigin, out progressingOrigin);
                }
                else
                    return false;
            ...
            while (true)
            {
                    ...
                    progressingOrigin.Y < lowest && localRay.Direction.Y < 0))
                {
                    ...
                    if (didHit1 && didHit2)
                    {
                        if (hit1.T < hit2.T)
                        {
                            Vector3.Mul(ref ray.Direction, hit1.T, out hit.Location);
                            Vector3.Add(ref hit.Location, ref ray.Position, out hit.Location);
                            Vector3.TransformTranspose(ref hit1.Normal, ref inverse.Transform, out hit.Normal);
                            hit.T = hit1.T;
                            return true;
                        }
                        Vector3.Mul(ref ray.Direction, hit2.T, out hit.Location);
                        Vector3.Add(ref hit.Location, ref ray.Position, out hit.Location);
                        Vector3.TransformTranspose(ref hit2.Normal, ref inverse.Transform, out hit.Normal);
                        hit.T = hit2.T;
                        return true;
                    }
                    else if (didHit1)
                ...
                if (timeToX < timeToZ)
                {
                    ...
                    Vector3 increment;
                    Vector3.Mul(ref localRay.Direction, timeToX, out increment);
                    Vector3.Add(ref increment, ref progressingOrigin, out progressingOrigin);
                }
                else
                {
                    ...
                    Vector3 increment;
                    Vector3.Mul(ref localRay.Direction, timeToZ, out increment);
                    Vector3.Add(ref increment, ref progressingOrigin, out progressingOrigin);
                }
        ...
        public void GetPosition(int i, int j, ref AffineTransform3 transform, out Vector3 position)
        {
            if (i <= 0)
                i = 0;
        ...
        public void GetNormal(int i, int j, ref AffineTransform3 transform, out Vector3 normal)
        {
            Vector3 top;
            Vector3 bottom;
            Vector3 right;
            ...
            Vector3 temp;
            Vector3.Sub(ref top, ref bottom, out temp);
            Vector3.Sub(ref right, ref left, out normal);
        ...
        public bool GetOverlaps(BoundingBox3 localSpaceBoundingBox, RawList<TriangleMeshConvexContactManifold.TriangleIndices> overlappedTriangles)
        {
        ...
        public bool GetOverlaps(BoundingBox3 localBoundingBox, RawList<int> overlappedElements)
        {
            int width = heights.GetLength(0);
        ...
        public void GetTriangle(ref TriangleMeshConvexContactManifold.TriangleIndices indices, ref AffineTransform3 transform, out Vector3 a, out Vector3 b, out Vector3 c)
        {
        ...
        ///<param name="b">Second vertex of the triangle.</param>
        ///<param name="c">Third vertex of the triangle.</param>
        public void GetTriangle(int index, ref AffineTransform3 transform, out Vector3 a, out Vector3 b, out Vector3 c)
        {
            //Find the quad.
