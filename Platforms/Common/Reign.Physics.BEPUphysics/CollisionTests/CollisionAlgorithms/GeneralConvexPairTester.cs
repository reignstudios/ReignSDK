...
namespace BEPUphysics.CollisionTests.CollisionAlgorithms
{
    ...
    public class GeneralConvexPairTester
    {
        ...
        private bool DoShallowContact(out ContactData contact)
        {
            ...
            //Vector3 sub;
            //Vector3.Sub(ref closestA, ref closestB, out sub);
            ...
            if (distanceSquared < margin * margin)
            {
                ...
                if (margin > Toolbox.Epsilon) //Avoid a NaN!
                    Vector3.Mul(ref displacement, collidableA.Shape.collisionMargin / margin, out contact.Position); //t * AB
                else
        ...
        private bool DoDeepContact(out ContactData contact)
        {
            ...
            if (previousState == CollisionState.Separated) //If it was shallow before, then its closest points will be used to find the normal.
            {
                ...
                //Vector3 velocityB;
                //Vector3.Sub(ref contact.Position, ref localTransformB.Position, out velocityB);
                ...
                //Vector3.Sub(ref velocityA, ref velocityB, out localDirection);
                ...
                if (collidableA.entity != null && collidableB.entity != null)
                    Vector3.Sub(ref collidableA.entity.linearVelocity, ref collidableB.entity.linearVelocity, out localDirection);
                else
            ...
            //    {
            //        Vector3.Div(ref localDirection, (float)Math.Sqrt(lengthSquared), out rayCastDirection);// (Vector3.Normalize(localDirection) + Vector3.Normalize(collidableB.worldTransform.Position - collidableA.worldTransform.Position)) / 2;
            ...
            //    Matrix3 orientation;
            //    Matrix3.FromQuaternion(ref collidableA.worldTransform.Orientation, out orientation);
            ...
            //    MPRTesting.LocalSurfaceCast(collidableA.Shape, collidableB.Shape, ref localTransformB, ref rayCastDirection, out contact.PenetrationDepth, out contact.Normal);
            //    MPRTesting.LocalSurfaceCast(collidableA.Shape, collidableB.Shape, ref localTransformB, ref contact.Normal, out contact.PenetrationDepth, out rayCastDirection);
            //    Vector3.Transform(ref contact.Position, ref collidableA.worldTransform, out contact.Position);
