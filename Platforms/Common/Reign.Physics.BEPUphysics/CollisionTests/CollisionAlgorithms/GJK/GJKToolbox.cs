...
namespace BEPUphysics.CollisionTests.CollisionAlgorithms.GJK
{
    ...
    public static class GJKToolbox
    {
        ...
        public static bool AreShapesIntersecting(ConvexShape shapeA, ConvexShape shapeB, ref RigidTransform3 transformA, ref RigidTransform3 transformB)
        {
                                                 ...
                                                 ref Vector3 localSeparatingAxis)
        {
            ...
            while (count++ < MaximumGJKIterations)
            {
                ...
                Vector3 direction;
                Vector3.Neg(ref closestPoint, out direction);
        ...
        public static bool GetClosestPoints(ConvexShape shapeA, ConvexShape shapeB, ref RigidTransform3 transformA, ref RigidTransform3 transformB,
                                            out Vector3 closestPointA, out Vector3 closestPointB)
        {
            ...
            Vector3.Transform(ref closestPointA, ref transformA, out closestPointA);
            Vector3.Transform(ref closestPointB, ref transformA, out closestPointB);
            return toReturn;
        }
        ...
        private static bool GetClosestPoints(ConvexShape shapeA, ConvexShape shapeB, ref RigidTransform3 localTransformB,
        ...
        ///</summary>
        ///<param name="ray">Ray3 to test against the shape.</param>
        ...
        public static bool RayCast(Ray3 ray, ConvexShape shape, ref RigidTransform3 shapeTransform, float maximumLength,
                                   out RayHit hit)
        {
            //Transform the ray into the object's local space.
            Vector3.Sub(ref ray.Position, ref shapeTransform.Position, out ray.Position);
            ...
            while (v.LengthSquared() >= Toolbox.Epsilon * simplex.GetErrorTolerance(ref ray.Position))
            {
                ...
                Vector3.Sub(ref hit.Location, ref p, out w);
        ...
        public static bool ConvexCast(ConvexShape sweptShape, ConvexShape target, ref Vector3 sweep, ref RigidTransform3 startingSweptTransform, ref RigidTransform3 targetTransform,
                                  out RayHit hit)
        {
        ...
        public static bool ConvexCast(ConvexShape shapeA, ConvexShape shapeB, ref Vector3 sweepA, ref Vector3 sweepB, ref RigidTransform3 transformA, ref RigidTransform3 transformB,
                                  out RayHit hit)
        {
            //Put the velocity into shapeA's local space.
            Vector3 velocityWorld;
            Vector3.Sub(ref sweepB, ref sweepA, out velocityWorld);
            ...
            Vector3.Sub(ref transformB.Position, ref transformA.Position, out localTransformB.Position);
            ...
            do
            {
                ...
                if (vw > 0)
                {
                    ...
                    //Shift the ray up.
                    Vector3.Mul(ref rayDirection, hit.T, out hit.Location);
            ...
            Vector3.Mul(ref velocityWorld, hit.T, out hit.Location);
        ...
        public static bool SphereCast(Ray3 ray, float radius, ConvexShape shape, ref RigidTransform3 shapeTransform, float maximumLength,
                                   out RayHit hit)
        {
            //Transform the ray into the object's local space.
            Vector3.Sub(ref ray.Position, ref shapeTransform.Position, out ray.Position);
        ...
        ///<param name="hit">Hit data of the sphere cast, if any.</param>
        ///<returns>Whether or not the sphere cast hit the shape.</returns>
        public static bool CCDSphereCast(Ray3 ray, float radius, ConvexShape target, ref RigidTransform3 shapeTransform, float maximumLength,
                                   out RayHit hit)
        {
            int iterations = 0;
            while (true)
