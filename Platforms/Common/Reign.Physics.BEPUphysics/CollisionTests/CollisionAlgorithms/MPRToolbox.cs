...
namespace BEPUphysics.CollisionTests.CollisionAlgorithms
{
    ...
    public static class MPRToolbox
    {
        ...
        public static bool GetOverlapPosition(ConvexShape shapeA, ConvexShape shapeB, ref RigidTransform3 transformA, ref RigidTransform3 transformB, out Vector3 position)
        {
            RigidTransform3 localTransformB;
            ...
            Vector3.Transform(ref position, ref transformA, out position);
            return toReturn;
        ...
        /// <returns>Whether or not the two shapes overlap.</returns>
        public static bool GetLocalOverlapPosition(ConvexShape shapeA, ConvexShape shapeB, ref RigidTransform3 localTransformB, out Vector3 position)
        {
            return GetLocalOverlapPosition(shapeA, shapeB, ref localTransformB.Position, ref localTransformB, out position);
        }
        
        internal static bool GetLocalOverlapPosition(ConvexShape shapeA, ConvexShape shapeB, ref Vector3 originRay, ref RigidTransform3 localTransformB, out Vector3 position)
        {
            ...
            Vector3 v0;
            Vector3.Neg(ref originRay, out v0); //Since we're in A's local space, A-B is just -B.
            ...
            if (n.LengthSquared() < Toolbox.Epsilon)
            {
                ...
                //Vector3.Sub(ref v1A, ref v0A, out offset); //'v0a' is just the zero vector, so there's no need to calculate the offset.
                Vector3.Mul(ref v1A, barycentricCoordinate, out position);
                return true;
            }
            ...
            Vector3.Sub(ref v1, ref v0, out temp1);
            Vector3.Sub(ref v2, ref v0, out temp2);
            ...
            while (true)
            {
                ...
                if (dot < 0)
                {
                    ...
                    Vector3.Sub(ref v1, ref v0, out temp1);
                    Vector3.Sub(ref v3, ref v0, out temp2);
                    Vector3.Cross(ref temp1, ref temp2, out n);
                    continue;
                }
                ...
                if (dot < 0)
                {
                    ...
                    v1B = v3B;
                    // Calculate the normal of the plane that will be used to find a new extreme point.
                    Vector3.Sub(ref v2, ref v0, out temp1);
                    Vector3.Sub(ref v3, ref v0, out temp2);
                    Vector3.Cross(ref temp1, ref temp2, out n);
                    continue;
                }
            ...
            while (true)
            {
                ...
                Vector3.Sub(ref v3, ref v2, out temp1);
                Vector3.Sub(ref v1, ref v2, out temp2);
                Vector3.Cross(ref temp1, ref temp2, out n);
                float dot;
                ...
                if (dot >= 0)
                {
                    ...
                    Vector3.Sub(ref v1, ref v0, out temp1);
                    Vector3.Sub(ref v2, ref v0, out temp2);
                    Vector3.Sub(ref v3, ref v0, out temp3);
        ...
        public static bool AreShapesOverlapping(ConvexShape shapeA, ConvexShape shapeB, ref RigidTransform3 transformA, ref RigidTransform3 transformB)
        {
            RigidTransform3 localTransformB;
            MinkowskiToolbox.GetLocalTransform(ref transformA, ref transformB, out localTransformB);
        ...
        /// <returns>Whether or not the shapes are overlapping.</returns>
        public static bool AreLocalShapesOverlapping(ConvexShape shapeA, ConvexShape shapeB, ref RigidTransform3 localTransformB)
        {
        ...
        /// <param name="localTransformB">Relative transform of shape B to shape A.</param>
        /// <returns>Whether or not the shapes are overlapping.</returns>
        internal static bool AreLocalShapesOverlapping(ConvexShape shapeA, ConvexShape shapeB, ref Vector3 originRay, ref RigidTransform3 localTransformB)
        {
            //Compute the origin ray.  This points from a point known to be inside the minkowski sum to the origin.
            //The centers of the shapes are used to create the interior point.
        ...
        public static void LocalSurfaceCast(ConvexShape shapeA, ConvexShape shapeB, ref RigidTransform3 localTransformB, ref Vector3 direction, out float t, out Vector3 normal)
        {
            ...
            if (n.LengthSquared() < Toolbox.Epsilon)
            {
                ...
                if (rayLengthSquared > Toolbox.Epsilon * .01f)
                    Vector3.Div(ref direction, (float)Math.Sqrt(rayLengthSquared), out normal);
                else
                    normal = new Vector3();
            ...
            {
                //Compute the outward facing normal.
                Vector3.Sub(ref v1, ref v2, out temp1);
                Vector3.Sub(ref v3, ref v2, out temp2);
                Vector3.Cross(ref temp1, ref temp2, out n);
                ...
                if (supportDot - dot < surfaceEpsilon || count > MPRToolbox.InnerIterationLimit) // TODO: Could use a dynamic epsilon for possibly better behavior.
                {
                    ...
                    //Vector3.Mul(ref normal, normalLengthInverse, out normal);
                    ...
                    {
                        Vector3.Div(ref n, (float)Math.Sqrt(lengthSquared), out normal);
        ...
        public static void LocalSurfaceCast(ConvexShape shapeA, ConvexShape shapeB, ref RigidTransform3 localTransformB, ref Vector3 direction, out float t, out Vector3 normal, out Vector3 position)
        {
            // Local surface cast is very similar to regular MPR.  However, instead of starting at an interior point and targeting the origin,
            // the ray starts at the origin (a point known to be in both shapeA and shapeB), and just goes towards the direction until the surface
            // is found.  The portal (v1, v2, v3) at termination defines the surface normal, and the distance from the origin to the portal along the direction is used as the 't' result.
            ...
            while (true)
            {
                ...
                if (supportDot - dot < surfaceEpsilon || count > MPRToolbox.InnerIterationLimit) // TODO: Could use a dynamic epsilon for possibly better behavior.
                {
                    ...
                    Vector3.Mul(ref direction, t, out position);

                    Toolbox.GetBarycentricCoordinates(ref position, ref v1, ref v2, ref v3, out v1Weight, out v2Weight, out v3Weight);
                    Vector3.Mul(ref v1A, v1Weight, out position);
                    Vector3 temp;
                    Vector3.Mul(ref v2A, v2Weight, out temp);
                    Vector3.Add(ref temp, ref position, out position);
                    Vector3.Mul(ref v3A, v3Weight, out temp);
                    Vector3.Add(ref temp, ref position, out position);
                    ////DEBUG STUFF:
        ...
        static bool VerifySimplex(ref Vector3 v0, ref Vector3 v1, ref Vector3 v2, ref Vector3 v3, ref Vector3 direction)
        {
            ...
            //v1, v0, v3
            Vector3 cross = (v0 - v1).Cross(v3 - v1);
            float planeProduct1 = cross.Dot(direction);
            //v3, v0, v2
            cross = (v0 - v3).Cross(v2 - v3);
            float planeProduct2 = cross.Dot(direction);
            //v2, v0, v1
            cross = (v0 - v2).Cross(v1 - v2);
            float planeProduct3 = cross.Dot(direction);
        ...
        public static bool GetContact(ConvexShape shapeA, ConvexShape shapeB, ref RigidTransform3 transformA, ref RigidTransform3 transformB, ref Vector3 penetrationAxis, out ContactData contact)
        {
            RigidTransform3 localTransformB;
            MinkowskiToolbox.GetLocalTransform(ref transformA, ref transformB, out localTransformB);
            if (MPRToolbox.AreLocalShapesOverlapping(shapeA, shapeB, ref localTransformB))
            {
                ...
                {
                    Vector3.Div(ref penetrationAxis, (float)Math.Sqrt(lengthSquared), out rayCastDirection);// (Vector3.Normalize(localDirection) + Vector3.Normalize(collidableB.worldTransform.Position - collidableA.worldTransform.Position)) / 2;
                ...
                if (lengthSquared > Toolbox.Epsilon)
                {
                    Vector3.Div(ref localTransformB.Position, (float)Math.Sqrt(lengthSquared), out rayCastDirection);
                ...
                Matrix3 orientation;
                Matrix3.FromQuaternion(ref transformA.Orientation, out orientation);
                Vector3.Transform(ref contact.Normal, ref orientation, out contact.Normal);
                //Vector3.Neg(ref contact.Normal, out contact.Normal);
                Vector3.Transform(ref contact.Position, ref orientation, out contact.Position);
        ...
        public static void RefinePenetration(ConvexShape shapeA, ConvexShape shapeB, ref RigidTransform3 localTransformB, float initialDepth, ref Vector3 initialNormal, out float penetrationDepth, out Vector3 refinedNormal, out Vector3 position)
        {
            //The local casting can optionally continue.  Eventually, it will converge to the local minimum.
        ...
        public static bool Sweep(ConvexShape shapeA, ConvexShape shapeB, ref Vector3 sweepA, ref Vector3 sweepB, ref RigidTransform3 transformA, ref RigidTransform3 transformB, out RayHit hit)
        {
            ...
            Vector3.Sub(ref sweepA, ref sweepB, out velocityWorld);
            ...
            if (rayLengthSquared > Toolbox.Epsilon * .01f)
            {
                ...
                //Scale the sweep length by the margins.  Div by the length to pull the margin into terms of the length of the ray.
            ...
            Vector3 sweep;
            Vector3.Mul(ref localDirection, sweepLength, out sweep);
            ...
            if (LocalSweepCast(shapeA, shapeB, sweepLength, rayLengthSquared, ref localDirection, ref sweep, ref localTransformB, out hit))
            {
                ...
                Vector3.Transform(ref hit.Location, ref transformA, out hit.Location);
                Vector3.Transform(ref hit.Normal, ref transformA.Orientation, out hit.Normal);
                //Push the world space hit location relative to object A along A's sweep direction.
                Vector3 temp;
                Vector3.Mul(ref sweepA, hit.T, out temp);
        ...
        private static bool LocalSweepCast(ConvexShape shapeA, ConvexShape shapeB, float sweepLength, float rayLengthSquared, ref Vector3 localDirection, ref Vector3 sweep, ref RigidTransform3 localTransformB, out RayHit hit)
        {
            ...
            if (n.LengthSquared() < Toolbox.Epsilon * .01f)
            {
                ...
                if (rayLengthSquared > Toolbox.Epsilon * .01f)
                    Vector3.Div(ref localDirection, (float)Math.Sqrt(rayLengthSquared), out hit.Normal);
                else
                    hit.Normal = new Vector3();
            ...
            while (true)
            {
                ...
                if (supportDot - dot < rayCastSurfaceEpsilon || count > MPRToolbox.InnerIterationLimit) // TODO: Could use a dynamic epsilon for possibly better behavior.
                {
                    ...
                    {
                        Vector3.Div(ref n, (float)Math.Sqrt(lengthSquared), out hit.Normal);
        ...
        internal static void GetLocalPosition(ConvexShape shapeA, ConvexShape shapeB, ref RigidTransform3 localTransformB, ref Vector3 minkowskiPosition, out Vector3 position)
        {
            ...
            Vector3 v0;
            Vector3.Neg(ref localTransformB.Position, out v0); //Since we're in A's local space, A-B is just -B.
            ...
            if (n.LengthSquared() < Toolbox.Epsilon)
            {
                ...
                float dot = (v1 - minkowskiPosition).Dot(rayDirection);
                ...
                //Origin is inside.
                //Compute barycentric coordinates along simplex (segment).
                float dotv0 = (v0 - minkowskiPosition).Dot(rayDirection);
                //Dot > 0, so dotv0 starts out negative.
                ...
                Vector3.Mul(ref v1A, barycentricCoordinate, out position);
                Vector3 offset;
                Vector3.Sub(ref v1B, ref localTransformB.Position, out offset);
                return;
            }
            MinkowskiToolbox.GetLocalMinkowskiExtremePoint(shapeA, shapeB, ref n, ref localTransformB, out v2A, out v2B, out v2);
            ...
            Vector3 v0v1, v0v2;
            //Set n for the first iteration.
            Vector3.Sub(ref v1, ref v0, out v0v1);
            Vector3.Sub(ref v2, ref v0, out v0v2);
            Vector3.Cross(ref v0v1, ref v0v2, out n);

            Vector3 pointToV0;
            Vector3.Sub(ref v0, ref minkowskiPosition, out pointToV0);
            Vector3 v3A, v3B, v3;
            int count = 0;
            while (true)
            {
                ...
                Vector3 v0v3;
                Vector3.Sub(ref v1, ref v0, out v0v1);
                Vector3.Sub(ref v3, ref v0, out v0v3);
            ...
            while (true)
            {
                ...
                //Compute the outward facing normal.
                Vector3 v2v3, v2v1;
                Vector3.Sub(ref v3, ref v2, out v2v3);
                Vector3.Sub(ref v1, ref v2, out v2v1);
                Vector3.Cross(ref v2v3, ref v2v1, out n);
                float dot;
                Vector3 pointToV1;
                Vector3.Sub(ref v1, ref minkowskiPosition, out pointToV1);
                ...
                float dot2;
                Vector3 pointToV4;
                Vector3.Sub(ref v4, ref minkowskiPosition, out pointToV4);
                ...
                if (dot2 - dot < rayCastSurfaceEpsilon || count > MPRToolbox.InnerIterationLimit) // TODO: Could use a dynamic epsilon for possibly better behavior.
                {
                    ...
                    Vector3.Mul(ref v1A, weight1, out position);
                    Vector3.Mul(ref v2A, weight2, out v2A);
                    Vector3.Mul(ref v3A, weight3, out v3A);
                ...
                {
                    Vector3 pointToV2;
                    Vector3.Sub(ref v2, ref minkowskiPosition, out pointToV2);
                ...
                {
                    Vector3 pointToV3;
                    Vector3.Sub(ref v3, ref minkowskiPosition, out pointToV3);
        ...
        public static bool AreSweptShapesIntersecting(ConvexShape shapeA, ConvexShape shapeB, ref Vector3 sweep, ref RigidTransform3 localTransformB, out Vector3 position)
        {
            //It's possible that the two objects' centers are overlapping, or very very close to it.  In this case, 
            //they are obviously colliding and we can immediately exit.
        ...
        static void GetSweptExtremePoint(ConvexShape shapeA, ConvexShape shapeB, ref RigidTransform3 localTransformB, ref Vector3 sweep, ref Vector3 extremePointDirection, out Vector3 extremePoint)
        {
        ...
        static void GetSweptExtremePoint(ConvexShape shapeA, ConvexShape shapeB, ref RigidTransform3 localTransformB, ref Vector3 sweep, ref Vector3 extremePointDirection, out Vector3 extremePointA, out Vector3 extremePoint)
        {
            Vector3 b;
