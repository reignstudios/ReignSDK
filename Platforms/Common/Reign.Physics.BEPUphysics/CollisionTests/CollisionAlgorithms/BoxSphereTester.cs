...
namespace BEPUphysics.CollisionTests.CollisionAlgorithms
{
    ...
    public static class BoxSphereTester
    {
        ...
        public static bool AreShapesColliding(BoxShape box, SphereShape sphere, ref RigidTransform3 boxTransform, ref Vector3 spherePosition, out ContactData contact)
        {
            contact = new ContactData();

            Vector3 localPosition;
            Vector3.TransformInversed(ref spherePosition, ref boxTransform, out localPosition);

            Vector3 localClosestPoint = new Vector3();
            localClosestPoint.X = MathUtilities.Clamp(localPosition.X, -box.halfWidth, box.halfWidth);
            localClosestPoint.Y = MathUtilities.Clamp(localPosition.Y, -box.halfHeight, box.halfHeight);
            localClosestPoint.Z = MathUtilities.Clamp(localPosition.Z, -box.halfLength, box.halfLength);

            Vector3.Transform(ref localClosestPoint, ref boxTransform, out contact.Position);

            Vector3 offset;
            Vector3.Sub(ref spherePosition, ref contact.Position, out offset);
            ...
            if (offsetLength > Toolbox.Epsilon)
            {
                ...
                //Outside of the box.
                Vector3.Div(ref offset, offsetLength, out contact.Normal);
