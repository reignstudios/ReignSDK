...
namespace BEPUphysics.CollisionTests.CollisionAlgorithms
{
    ...
    public class TriangleConvexPairTester : TrianglePairTester
    {
        ...
        internal CollisionState state = CollisionState.Plane3;
        ...
        private bool DoPlaneTest(out TinyStructList<ContactData> contactList)
        {
            ...
            Vector3 ab, ac;
            Vector3.Sub(ref triangle.vB, ref triangle.vA, out ab);
            Vector3.Sub(ref triangle.vC, ref triangle.vA, out ac);
            ...
            switch (triangle.sidedness)
            {
                case TriangleSidedness.DoubleSided:
                    if (dotA < 0)
                    {
                        ...
                        Vector3.Neg(ref reverseNormal, out reverseNormal);
                        dotA = -dotA;
                    }
                    break;
            ...
            Vector3 offset;
            Vector3.Mul(ref reverseNormal, t, out offset);
            ...
            if (t <= 0 || distanceSquared < marginSum * marginSum)
            {
                ...
                if (marginSum > Toolbox.Epsilon) //This can be zero! It would cause a NaN is unprotected.
                    Vector3.Mul(ref offset, convex.collisionMargin / marginSum, out contact.Position); //t * AB
                ...
                Vector3.Div(ref reverseNormal, normalLength, out contact.Normal);
        ...
        private bool DoExternalNear(out TinyStructList<ContactData> contactList)
        {
            ...
            Vector3.Mul(ref triangleCentroid, .33333333f, out triangleCentroid);
            ...
            Vector3.Sub(ref closestB, ref closestA, out displacement);
            ...
            if (distanceSquared < margin * margin)
            {
                ...
                if (margin > Toolbox.Epsilon) //This can be zero! It would cause a NaN if unprotected.
                    Vector3.Mul(ref displacement, convex.collisionMargin / margin, out contact.Position); //t * AB
                else contact.Position = new Vector3();
                ...
                Vector3.Div(ref contact.Normal, distance, out contact.Normal);
        ...
        private bool DoDeepContact(out TinyStructList<ContactData> contactList)
        {
            ...
            if (MPRToolbox.AreLocalShapesOverlapping(convex, triangle, ref center, ref Toolbox.RigidIdentity))
            {
                ...
                else
                {
                    ...
                    Vector3.Div(ref triangleNormal, (float)Math.Sqrt(lengthSquared), out triangleNormal);
                    ...
                    //Vector3.Mul(ref triangleNormal, dot, out trianglePlaneDirection);
                    //Vector3.Sub(ref trianglePlaneDirection, ref center, out trianglePlaneDirection);
                    ...
                    //    Vector3 AB, AO;
                    //    Vector3.Sub(ref triangle.vB, ref triangle.vA, out AB);
                    //    Vector3.Sub(ref center, ref triangle.vA, out AO);
                    ...
                    //    Vector3 BC, BO;
                    //    Vector3.Sub(ref triangle.vC, ref triangle.vB, out BC);
                    //    Vector3.Sub(ref center, ref triangle.vB, out BO);
                    ...
                    //    Vector3 CA, CO;
                    //    Vector3.Sub(ref triangle.vA, ref triangle.vC, out CA);
                    //    Vector3.Sub(ref center, ref triangle.vC, out CO);
                    ...
                    //{
                    //    Vector3.Div(ref trianglePlaneDirection, (float)Math.Sqrt(dot), out trianglePlaneDirection);
                    ...
                    //        Vector3.Mul(ref contact.Normal, dot, out p);
                    //        Vector3.Sub(ref contact.Normal, ref p, out contact.Normal);
                    //        float length = contact.Normal.LengthSquared();
                    //        if (length > Toolbox.Epsilon)
                    //        {
                    //            //Renormalize the corrected normal.
                    //            Vector3.Div(ref contact.Normal, (float)Math.Sqrt(length), out contact.Normal);
                    ...
                    Vector3.Mul(ref AB, dot / AB.LengthSquared(), out ABnormal);
                    Vector3.Sub(ref AO, ref ABnormal, out ABnormal);
                    ABnormal = ABnormal.Normalize();

                    //Project the center onto the edge to find the direction from the center to the edge BC.
                    Vector3.Dot(ref BO, ref BC, out dot);
                    Vector3.Mul(ref BC, dot / BC.LengthSquared(), out BCnormal);
                    Vector3.Sub(ref BO, ref BCnormal, out BCnormal);
                    BCnormal = BCnormal.Normalize();

                    //Project the center onto the edge to find the direction from the center to the edge BC.
                    Vector3.Dot(ref CO, ref CA, out dot);
                    Vector3.Mul(ref CA, dot / CA.LengthSquared(), out CAnormal);
                    Vector3.Sub(ref CO, ref CAnormal, out CAnormal);
                    CAnormal = CAnormal.Normalize();
                    ...
                    if ((triangle.sidedness == TriangleSidedness.Clockwise && dot > 0) || (triangle.sidedness == TriangleSidedness.Counterclockwise && dot < 0))
                    {
                        ...
                        Vector3 p;
                        Vector3.Mul(ref candidateNormal, dot, out p);
                        Vector3.Sub(ref candidateNormal, ref p, out candidateNormal);
                        float length = candidateNormal.LengthSquared();
                        if (length > Toolbox.Epsilon)
                        {
                            //Renormalize the corrected normal.
                            Vector3.Div(ref candidateNormal, (float)Math.Sqrt(length), out candidateNormal);
                    ...
                    {
                        Vector3.Neg(ref triangleNormal, out triangleNormal);
                        MPRToolbox.LocalSurfaceCast(convex, triangle, ref Toolbox.RigidIdentity, ref triangleNormal, out candidateDepth, out candidateNormal);
                        if (candidateDepth < contact.PenetrationDepth)
                        {
                            contact.Normal = candidateNormal;
        ...
        private bool TryInnerSphereContact(out ContactData contact)
        {
            ...
            if (length < minimumRadius * minimumRadius)
            {
                ...
                {
                    Vector3.Div(ref closestPoint, length, out contact.Normal);
                }
                else
                {
                    ...
                    {
                        Vector3.Div(ref triangleNormal, (float)Math.Sqrt(normalLength), out triangleNormal);
                        if (triangle.sidedness == TriangleSidedness.Clockwise)
                            contact.Normal = triangleNormal;
                        else
                            Vector3.Neg(ref triangleNormal, out contact.Normal);
                    }
                    else
                    {
                        //Degenerate triangle!
        ...
        private VoronoiRegion GetVoronoiRegion(ref Vector3 p)
        {
            ...
            Vector3.Sub(ref triangle.vB, ref triangle.vA, out ab);
            Vector3.Sub(ref triangle.vC, ref triangle.vA, out ac);
            Vector3.Sub(ref p, ref triangle.vA, out ap);
            ...
            Vector3 bp;
            Vector3.Sub(ref p, ref triangle.vB, out bp);
            ...
            Vector3 cp;
            Vector3.Sub(ref p, ref triangle.vC, out cp);
        ...
        public override VoronoiRegion GetRegion(ref ContactData contact)
        {
            ...
            if (dotA > dotB && dotA > dotC)
            {
                ...
                if (dotB > dotC)
                {
                    ...
                    {
                        Vector3.Sub(ref triangle.vB, ref triangle.vA, out edgeDirection);
                ...
                else
                {
                    ...
                    {
                        Vector3.Sub(ref triangle.vC, ref triangle.vA, out edgeDirection);
                        Vector3.Dot(ref edgeDirection, ref contact.Normal, out edgeDot);
                        if (edgeDot * edgeDot < edgeDirection.LengthSquared() * edgeEpsilon)
                            return VoronoiRegion.AC;
                        else
            ...
            else if (dotB > dotC)
            {
                ...
                if (dotC > dotA)
                {
                    ...
                    {
                        Vector3.Sub(ref triangle.vC, ref triangle.vB, out edgeDirection);
                        Vector3.Dot(ref edgeDirection, ref contact.Normal, out edgeDot);
                        if (edgeDot * edgeDot < edgeDirection.LengthSquared() * edgeEpsilon)
                            return VoronoiRegion.BC;
                        else
                ...
                else
                {
                    ...
                    {
                        Vector3.Sub(ref triangle.vA, ref triangle.vB, out edgeDirection);
                        Vector3.Dot(ref edgeDirection, ref contact.Normal, out edgeDot);
                        if (edgeDot * edgeDot < edgeDirection.LengthSquared() * edgeEpsilon)
                            return VoronoiRegion.AB;
                        else
            ...
            else
            {
                ...
                if (dotA > dotB)
                {
                    ...
                    {
                        Vector3.Sub(ref triangle.vA, ref triangle.vC, out edgeDirection);
                        Vector3.Dot(ref edgeDirection, ref contact.Normal, out edgeDot);
                        if (edgeDot * edgeDot < edgeDirection.LengthSquared() * edgeEpsilon)
                            return VoronoiRegion.AC;
                        else
                ...
                else
                {
                    ...
                    {
                        Vector3.Sub(ref triangle.vB, ref triangle.vC, out edgeDirection);
                        Vector3.Dot(ref edgeDirection, ref contact.Normal, out edgeDot);
                        if (edgeDot * edgeDot < edgeDirection.LengthSquared() * edgeEpsilon)
                            return VoronoiRegion.BC;
                        else
