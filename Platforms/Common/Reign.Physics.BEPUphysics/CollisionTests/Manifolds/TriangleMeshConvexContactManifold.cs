...
namespace BEPUphysics.CollisionTests.Manifolds
{
    ...
    public abstract class TriangleMeshConvexContactManifold : ContactManifold
    {
        ...
        protected virtual RigidTransform3 MeshTransform
        {
            get
            {
        ...
        public override void Update(float dt)
        {
            ...
            Matrix3 orientation;
            Matrix3.FromQuaternion(ref convex.worldTransform.Orientation, out orientation);
            for (int i = 0; i < triangleCount; i++)
            {
                ...
                if (ConfigureTriangle(i, out indices))
                {
                    ...
                    Vector3.Sub(ref localTriangleShape.vA, ref convex.worldTransform.Position, out localTriangleShape.vA);
                    Vector3.Sub(ref localTriangleShape.vB, ref convex.worldTransform.Position, out localTriangleShape.vB);
                    Vector3.Sub(ref localTriangleShape.vC, ref convex.worldTransform.Position, out localTriangleShape.vC);
                    Vector3.TransformTranspose(ref localTriangleShape.vA, ref orientation, out localTriangleShape.vA);
                    Vector3.TransformTranspose(ref localTriangleShape.vB, ref orientation, out localTriangleShape.vB);
                    Vector3.TransformTranspose(ref localTriangleShape.vC, ref orientation, out localTriangleShape.vC);
            ...
            if (UseImprovedBoundaryHandling)
            {
                ...
                if (vertexContacts.count == 0 && guaranteedContacts == 0 && edgeContacts.count > 1)
                {
                    ...
                    edgeContacts.Elements[0].CorrectedNormal = edgeContacts.Elements[0].CorrectedNormal.Normalize();
                    float dot;
                    ...
                    if (allNormalsInSamePlane && atLeastOneNormalAgainst)
                    {
                        ...
                        for (int i = 1; i < edgeContacts.count; i++)
                        {
                            ...
                            edgeContacts.Elements[i].CorrectedNormal = edgeContacts.Elements[i].CorrectedNormal.Normalize();
                ...
                for (int i = 0; i < edgeContacts.count; i++)
                {
                    ...
                    else if (edgeContacts.Elements[i].ShouldCorrect || guaranteedContacts == 0)
                    {
                        ...
                        Vector3.Dot(ref edgeContacts.Elements[i].CorrectedNormal, ref edgeContacts.Elements[i].ContactData.Normal, out dot);
                        edgeContacts.Elements[i].ContactData.Normal = edgeContacts.Elements[i].CorrectedNormal;
                        edgeContacts.Elements[i].ContactData.PenetrationDepth *= Math.Max(0, dot); //Never cause a negative penetration depth.
                        AddLocalContact(ref edgeContacts.Elements[i].ContactData, ref orientation);
                    }
                ...
                for (int i = 0; i < vertexContacts.count; i++)
                {
                    ...
                    else if (vertexContacts.Elements[i].ShouldCorrect || guaranteedContacts == 0)
                    {
                        ...
                        //it will not interfere (and cause a bump or something).
                        float dot;
                        vertexContacts.Elements[i].CorrectedNormal = vertexContacts.Elements[i].CorrectedNormal.Normalize();
                        ...
                        vertexContacts.Elements[i].ContactData.PenetrationDepth *= Math.Max(0, dot); //Never cause a negative penetration depth.
                        AddLocalContact(ref vertexContacts.Elements[i].ContactData, ref orientation);
                    }
                    //If it's blocked AND it doesn't allow correction, ignore its existence.
        ...
        void AddLocalContact(ref ContactData contact, ref Matrix3 orientation)
        {
        ...
        protected void GetNormal(ref Vector3 uncorrectedNormal, out Vector3 normal)
        {
            ...
            Vector3 AB, AC;
            Vector3.Sub(ref localTriangleShape.vB, ref localTriangleShape.vA, out AB);
            Vector3.Sub(ref localTriangleShape.vC, ref localTriangleShape.vA, out AC);
            ...
            switch (localTriangleShape.sidedness)
            {
                case TriangleSidedness.DoubleSided:
                    ...
                    if (dot < 0)
                        Vector3.Neg(ref normal, out normal);
                    break;
        ...
        protected override void Add(ref ContactData contactCandidate)
        {
            ...
            Vector3.TransformInversed(ref contactCandidate.Position, ref convex.worldTransform, out supplement.LocalOffsetA);
            RigidTransform3 transform = MeshTransform;
            Vector3.TransformInversed(ref contactCandidate.Position, ref transform, out supplement.LocalOffsetB);
        ...
        private bool IsContactUnique(ref ContactData contactCandidate)
        {
            ...
            RigidTransform3 meshTransform = MeshTransform;
            for (int i = 0; i < contacts.count; i++)
            {
                ...
                if (distanceSquared < CollisionDetectionSettings.ContactMinimumSeparationDistanceSquared)
                {
                    ...
                    if (Math.Abs(distanceSquared) >= CollisionDetectionSettings.nonconvexNormalDotMinimum)
                    {
                        ...
                        Vector3.TransformInversed(ref contactCandidate.Position, ref convex.worldTransform, out supplementData.Elements[i].LocalOffsetA);
                        Vector3.TransformInversed(ref contactCandidate.Position, ref meshTransform, out supplementData.Elements[i].LocalOffsetB);
                        return false;
                    }
                }
            }
