...
namespace BEPUphysics.CollisionTests.Manifolds
{
    ...
    public class GeneralConvexContactManifold : ContactManifold
    {
        ...
        protected override void Add(ref ContactData contactCandidate)
        {
            ...
            Vector3.TransformInversed(ref contactCandidate.Position, ref collidableA.worldTransform, out supplement.LocalOffsetA);
            Vector3.TransformInversed(ref contactCandidate.Position, ref collidableB.worldTransform, out supplement.LocalOffsetB);
        ...
        private bool IsContactUnique(ref ContactData contactCandidate)
        {
            for (int i = 0; i < contacts.count; i++)
            {
                ...
                if (distanceSquared < CollisionDetectionSettings.ContactMinimumSeparationDistanceSquared)
                {
                    ...
                    Vector3.TransformInversed(ref contactCandidate.Position, ref collidableA.worldTransform, out supplementData.Elements[i].LocalOffsetA);
                    Vector3.TransformInversed(ref contactCandidate.Position, ref collidableB.worldTransform, out supplementData.Elements[i].LocalOffsetB);
                    return false;
                }
            }
            return true;
