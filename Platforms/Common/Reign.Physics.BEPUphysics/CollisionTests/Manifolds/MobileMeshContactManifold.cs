...
namespace BEPUphysics.CollisionTests.Manifolds
{
    ...
    public abstract class MobileMeshContactManifold : TriangleMeshConvexContactManifold
    {
        ...
        protected override RigidTransform3 MeshTransform
        {
        ...
        protected internal override int FindOverlappingTriangles(float dt)
        {
            ...
            AffineTransform3 transform = new AffineTransform3(mesh.worldTransform.Orientation, mesh.worldTransform.Position);
            ...
            if (mesh.entity != null)
                Vector3.Sub(ref transformedVelocity, ref mesh.entity.linearVelocity, out transformedVelocity);

            //The linear transform is known to be orientation only, so using the transpose is allowed.
            Vector3.TransformTranspose(ref transformedVelocity, ref transform.Transform, out transformedVelocity);
            Vector3.Mul(ref transformedVelocity, dt, out transformedVelocity);
        ...
        protected override bool ConfigureTriangle(int i, out TriangleIndices indices)
        {
            ...
            AffineTransform3.FromRigidTransform(ref mesh.worldTransform, out transform);
            Vector3.Transform(ref localTriangleShape.vA, ref transform, out localTriangleShape.vA);
            Vector3.Transform(ref localTriangleShape.vB, ref transform, out localTriangleShape.vB);
            Vector3.Transform(ref localTriangleShape.vC, ref transform, out localTriangleShape.vC);
            ...
            bool toReturn;
            BoundingBox3.Intersects(ref triangleAABB, ref convex.boundingBox, out toReturn);
            if (!toReturn)
            {
        ...
        protected override void ProcessCandidates(RawValueList<ContactData> candidates)
        {
            if (candidates.count == 0 && parentContactCount == 0 && Mesh.Shape.solidity == MobileMeshSolidity.Solid)
            {
                ...
                Matrix3 orientation;
                Matrix3.FromQuaternion(ref mesh.worldTransform.Orientation, out orientation);

                Ray3 ray;
                Vector3.Sub(ref convex.worldTransform.Position, ref mesh.worldTransform.Position, out ray.Position);
                Vector3.TransformTranspose(ref ray.Position, ref orientation, out ray.Position);

                //Cast from the current position back to the previous position.
                Vector3.Sub(ref lastValidConvexPosition, ref ray.Position, out ray.Direction);
                ...
                Vector3.Div(ref ray.Direction, (float)Math.Sqrt(rayDirectionLength), out ray.Direction);
                ...
                if (mesh.Shape.IsLocalRayOriginInMesh(ref ray, out hit))
                {
                    ...
                    Vector3.Transform(ref ray.Position, ref orientation, out newContact.Position);
                    Vector3.Add(ref newContact.Position, ref mesh.worldTransform.Position, out newContact.Position);

                    newContact.Normal = hit.Normal.Normalize();
                    ...
                    Vector3.Transform(ref newContact.Normal, ref orientation, out newContact.Normal);
