...
namespace BEPUphysics.Paths
{
    ...
    public class FiniteDifferenceSpline3D : HermiteCurve3D
    {
        ...
        protected override void ComputeTangents()
        {
            ...
            Vector3.Sub(ref next, ref current, out tangentA);
            Vector3.Mul(ref tangentA, .5f / (ControlPoints[1].Time - ControlPoints[0].Time), out tangentA);
            //Vector3.Mul(ref current, .5f / (controlPoints[0].time), out tangentB);
            ...
            for (int i = 1; i < ControlPoints.Count - 1; i++)
            {
                ...
                Vector3.Sub(ref next, ref current, out tangentA);
                Vector3.Sub(ref current, ref previous, out tangentB);
                Vector3.Mul(ref tangentA, .5f / (ControlPoints[i + 1].Time - ControlPoints[i].Time), out tangentA);
                Vector3.Mul(ref tangentB, .5f / (ControlPoints[i].Time - ControlPoints[i - 1].Time), out tangentB);
                Vector3.Add(ref tangentA, ref tangentB, out tangentA);
                tangents.Add(tangentA);
            }
            ...
            previous = current;
            current = next;
            Vector3.Neg(ref current, out tangentA);
            Vector3.Sub(ref current, ref previous, out tangentB);
            int currentIndex = ControlPoints.Count - 1;
            int previousIndex = currentIndex - 1;
            //Vector3.Mul(ref tangentA, .5f / (-controlPoints[currentIndex].time), out tangentA);
            Vector3.Mul(ref tangentB, .5f / (ControlPoints[currentIndex].Time - ControlPoints[previousIndex].Time), out tangentB);
            //Vector3.Add(ref tangentA, ref tangentB, out tangentA);
