...
namespace BEPUphysics.Paths
{
    ...
    public class CardinalSpline3D : HermiteCurve3D
    {
        ...
        {
            get { return tension; }
            set { tension = MathUtilities.Clamp(value, 0, 1); }
        }
        ...
        protected override void ComputeTangents()
        {
            ...
            for (int i = 1; i < ControlPoints.Count - 1; i++)
            {
                ...
                Vector3.Sub(ref next, ref previous, out tangent);
                Vector3.Mul(ref tangent, (1 - tension) / (ControlPoints[i + 1].Time - ControlPoints[i - 1].Time), out tangent);
