...
namespace BEPUphysics.Collidables.MobileCollidables
{
    ...
    public static class CompoundHelper
    {
                        ...
                        out ShapeDistributionInformation distributionInfoA, out ShapeDistributionInformation distributionInfoB)
        {
            ...
            if (SplitCompound(childContributions, splitPredicate, a.CollisionInformation, bCollidable, out distributionInfoA, out distributionInfoB, out weightA, out weightB))
            {
                ...
                if (a.CollisionInformation.children.count > 0)
                {
                    ...
                    Matrix3.Mul(ref distributionInfoA.VolumeDistribution, newMassA * InertiaHelper.InertiaTensorScale, out distributionInfoA.VolumeDistribution);
                ...
                if (bCollidable.children.count > 0)
                {
                    ...
                    Matrix3.Mul(ref distributionInfoB.VolumeDistribution, newMassB * InertiaHelper.InertiaTensorScale, out distributionInfoB.VolumeDistribution);
        ...
        static void SplitReposition(Entity a, Entity b, ref ShapeDistributionInformation distributionInfoA, ref ShapeDistributionInformation distributionInfoB, float weightA, float weightB)
        {
            ...
            Vector3.Mul(ref distributionInfoA.Center, weightA, out weightedA);
            Vector3.Mul(ref distributionInfoB.Center, weightB, out weightedB);
            Vector3 newLocalCenter;
            Vector3.Add(ref weightedA, ref weightedB, out newLocalCenter);
            Vector3.Div(ref newLocalCenter, weightA + weightB, out newLocalCenter);

            Vector3 localOffsetA;
            Vector3 localOffsetB;
            Vector3.Sub(ref distributionInfoA.Center, ref newLocalCenter, out localOffsetA);
            Vector3.Sub(ref distributionInfoB.Center, ref newLocalCenter, out localOffsetB);

            Vector3 originalPosition = a.position;

            b.Orientation = a.Orientation;
            Vector3 offsetA = localOffsetA.Transform(a.Orientation);
            Vector3 offsetB = localOffsetB.Transform(a.Orientation);
            ...
            a.LinearVelocity = originalLinearVelocity + originalAngularVelocity.Cross(offsetA);
            b.LinearVelocity = originalLinearVelocity + originalAngularVelocity.Cross(offsetB);
        }
            ...
            out float weightA, out float weightB)
        {
            ...
            for (int i = a.children.count - 1; i >= 0; i--)
            {
                ...
                Vector3.Mul(ref contribution.Center, child.Entry.Weight, out contribution.Center);
            ...
            if (weightA > 0)
                Vector3.Div(ref distributionInfoA.Center, weightA, out distributionInfoA.Center);

            if (weightB > 0)
                Vector3.Div(ref distributionInfoB.Center, weightB, out distributionInfoB.Center);
            ...
            Vector3 offsetA;
            Vector3.Neg(ref distributionInfoA.Center, out offsetA);
            Vector3 offsetB;
            Vector3.Neg(ref distributionInfoB.Center, out offsetB);
            ...
            for (int i = a.children.count - 1; i >= 0; i--)
            {
                ...
                Matrix3.Add(ref contribution.VolumeDistribution, ref distributionInfoA.VolumeDistribution, out distributionInfoA.VolumeDistribution);
            }
            for (int i = b.children.count - 1; i >= 0; i--)
            {
                var child = b.children.Elements[i];
                ...
                Matrix3.Add(ref contribution.VolumeDistribution, ref distributionInfoB.VolumeDistribution, out distributionInfoB.VolumeDistribution);
            }

            //Normalize the volume distribution.
            Matrix3.Mul(ref distributionInfoA.VolumeDistribution, 1 / weightA, out distributionInfoA.VolumeDistribution);
            Matrix3.Mul(ref distributionInfoB.VolumeDistribution, 1 / weightB, out distributionInfoB.VolumeDistribution);
        ...
        static void RemoveReposition(Entity compound, ref ShapeDistributionInformation distributionInfo, float weight, float removedWeight, ref Vector3 removedCenter)
        {
            ...
            //These can be used to reposition the objects in world space.
            Vector3 weightedA, weightedB;
            Vector3.Mul(ref distributionInfo.Center, weight, out weightedA);
            Vector3.Mul(ref removedCenter, removedWeight, out weightedB);
            Vector3 newLocalCenter;
            Vector3.Add(ref weightedA, ref weightedB, out newLocalCenter);
            Vector3.Div(ref newLocalCenter, weight + removedWeight, out newLocalCenter);

            Vector3 localOffset;
            Vector3.Sub(ref distributionInfo.Center, ref newLocalCenter, out localOffset);

            Vector3 originalPosition = compound.position;

            Vector3 offset = localOffset.Transform(compound.orientation);
            ...
            compound.LinearVelocity = originalLinearVelocity + originalAngularVelocity.Cross(offset);
        }
                        ...
                        out ShapeDistributionInformation distributionInfo)
        {
            ...
            if (RemoveChildFromCompound(compound.CollisionInformation, removalPredicate, childContributions, out distributionInfo, out weight, out removedWeight, out removedCenter))
            {
                ...
                if (compound.CollisionInformation.Children.Count > 0)
                {
                    ...
                    Matrix3.Mul(ref distributionInfo.VolumeDistribution, newMass * InertiaHelper.InertiaTensorScale, out distributionInfo.VolumeDistribution);
           ...
           out ShapeDistributionInformation distributionInfo, out float weight, out float removedWeight, out Vector3 removedCenter)
        {
            ...
            for (int i = compound.children.count - 1; i >= 0; i--)
            {
                ...
                if (removalPredicate(child))
                {
                    ...
                    Vector3 toAdd;
                    Vector3.Mul(ref entry.LocalTransform.Position, entry.Weight, out toAdd);
            ...
            if (removedWeight > 0)
            {
                Vector3.Div(ref removedCenter, removedWeight, out removedCenter);
            }
            ...
            //Average the center out.
            Vector3.Div(ref distributionInfo.Center, weight, out distributionInfo.Center);
            ...
            Vector3 offset;
            Vector3.Neg(ref distributionInfo.Center, out offset);
            ...
            for (int i = compound.children.count - 1; i >= 0; i--)
            {
                ...
                //Vector3.Add(ref entry.LocalTransform.Position, ref offsetA, out entry.LocalTransform.Position);
                Matrix3.Add(ref contribution.VolumeDistribution, ref distributionInfo.VolumeDistribution, out distributionInfo.VolumeDistribution);
            }

            //Normalize the volume distribution.
            Matrix3.Mul(ref distributionInfo.VolumeDistribution, 1 / weight, out distributionInfo.VolumeDistribution);
        ...
        public static CompoundCollidable CreatePartialCompoundCollidable(CompoundShape shape, IList<int> childIndices)
        {
            ...
            Vector3.Div(ref center, totalWeight, out center);
