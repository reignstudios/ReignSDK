    ...
    public class CompoundCollidable : EntityCollidable
    {
        ...
        public override void UpdateWorldTransform(ref Vector3 position, ref Quaternion orientation)
        {
            ...
            for (int i = 0; i < children.count; i++)
            {
                ...
                RigidTransform3.Transform(ref shapeList.Elements[children.Elements[i].shapeIndex].LocalTransform, ref worldTransform, out transform);
        ...
        protected internal override void UpdateBoundingBoxInternal(float dt)
        {
            ...
            boundingBox = hierarchy.Tree.BoundingBox3;
        ...
        /// <param name="rayHit">Hit location of the ray on the collidable, if any.</param>
        /// <returns>Whether or not the ray hit the collidable.</returns>
        public override bool RayCast(Ray3 ray, float maximumLength, Func<BroadPhaseEntry, bool> filter, out RayHit rayHit)
        {
            rayHit = new RayHit();
            if (filter(this))
            {
        ...
        public bool RayCast(Ray3 ray, float maximumLength, out RayCastResult rayHit)
        {
        ...
        /// <returns>Whether or not the ray hit the entry.</returns>
        public bool RayCast(Ray3 ray, float maximumLength, out RayHit rayHit, out CompoundChild hitChild)
        {
            rayHit = new RayHit();
            hitChild = null;
            var hitElements = Resources.GetCompoundChildList();
