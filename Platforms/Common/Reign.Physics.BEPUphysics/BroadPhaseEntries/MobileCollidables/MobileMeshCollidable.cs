...
namespace BEPUphysics.Collidables.MobileCollidables
{
    ...
    public class MobileMeshCollidable : EntityCollidable
    {
        ...
        public override bool RayCast(Ray3 ray, float maximumLength, out RayHit rayHit)
        {
            ...
            Ray3 localRay;
            Matrix3 orientation;
            Matrix3.FromQuaternion(ref worldTransform.Orientation, out orientation);
            Vector3.TransformTranspose(ref ray.Direction, ref orientation, out localRay.Direction);
            Vector3.Sub(ref ray.Position, ref worldTransform.Position, out localRay.Position);
            Vector3.TransformTranspose(ref localRay.Position, ref orientation, out localRay.Position);
            ...
            if (Shape.solidity == MobileMeshSolidity.Solid)
            {
                ...
                else
                {
                    if (rayHit.T < maximumLength)
                    {
                        ...
                        Vector3.Transform(ref rayHit.Normal, ref orientation, out rayHit.Normal);
                    }
                    else
                    {
        ...
        public override bool ConvexCast(ConvexShape castShape, ref RigidTransform3 startingTransform, ref Vector3 sweep, out RayHit hit)
        {
            if (Shape.solidity == MobileMeshSolidity.Solid)
            {
                //If the convex cast is inside the mesh and the mesh is solid, it should return t = 0.
                var ray = new Ray3() { Position = startingTransform.Position, Direction = Toolbox.UpVector };
            ...
            var transform = new AffineTransform3 {Translation = worldTransform.Position};
            Matrix3.FromQuaternion(ref worldTransform.Orientation, out transform.Transform);
            ...
            if (this.Shape.TriangleMesh.Tree.GetOverlaps(boundingBox, hitElements))
            {
                ...
                for (int i = 0; i < hitElements.Count; i++)
                {
                    ...
                    Vector3.Transform(ref tri.vA, ref transform, out tri.vA);
                    Vector3.Transform(ref tri.vB, ref transform, out tri.vB);
                    Vector3.Transform(ref tri.vC, ref transform, out tri.vC);
                    Vector3 center;
